...
| Expand shorthand | Both `db/expand-map`, `ProcessInpoint`, `ProcessExpander`, `with-tx` | `transaction::Normalizer` and `assess_forms_with_clock`, then `tiered_assessor`. Retain shared assessment; reconcile nested-map identity guards. |
...
| Publish durably | `update/fressianer`, `writer`; `log/Log.append`, `write-tail-descriptor`, `claim-log` | Rust-owned log/receipt objects and conditional publication in `storage::engine`. Retain durable-before-success invariant and stale-writer checks. |
...
|---|---|---|
| `model` | Exact values, datoms, schema and identity | Datomic `db`, `data`, attribute/datoms interfaces; existing Rust value/schema/identity logic. |
| `collections` | Generic persistent/ordered structures and cache bookkeeping mechanisms | `btset`, priority-map composition and current `SharedMap`; no database IDs, SQL or cache policy in generic collections. |
| `index` | Persistent covering trees, recent indexes, overlays, ordered traversal and merging | Datomic `index`, `btset`, `db/IndexSet`; retain current path sharing and selective cursors where comparison supports it. |
...
- **Nested maps (transaction):** source `expand-submap` uses an explicit `:db/id`
  before calling the anonymous-child guard. `has-unique-id?` tests identity
  uniqueness; the prose says “unique” more broadly. Rust's current guard applies
  to explicit nested IDs too and accepts identity or value uniqueness. Reconcile
  with the documented orphan-prevention purpose and focused expected-result
  cases; do not silently choose a looser or stricter interpretation.
- **Write pipeline (transactor):** recovered processing, encoding, log writing
  ...
  ordering but is not algorithmic/concurrency equivalence. Measure the full path
  and justify or repair the adaptation in Stage 3, not by assuming Rust makes
  pipelining unnecessary.
...
- **URI identity (model, open):** raw-string URI comparison differs from the
  component-aware value comparison delegated to `java.net.URI` in the source.
  Scheme/host case and escaped-hex spelling can affect lookup/upsert behavior.
  Repair logical equality, ordering and hashing together while preserving exact
  canonical representation and retry identity; this is not JVM wire parity.
...
connection validation queries. Guarded multi-reference publication and protection
epochs still need comparison with their engine consumers; small DDL alone is not
an architectural acceptance claim.
