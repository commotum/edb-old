...
/// Canonical root payloads plus subsequent cold canonical-node bytes. Metadata
/// bootstrap SQL is additionally visible in the enclosing OperationContext;
/// its decoded projection does not retain separate per-node byte counters.
pub(super) fn source_node_bytes(snapshot: &TieredSnapshot) -> u64 {
    let roots: u64 = snapshot.state.tree_base.as_ref().map_or(0, |base| {
        base.manifest.trees.iter().map(|tree| tree.root_bytes).sum()
    });
    roots.saturating_add(lock(&snapshot.core.block_reads).canonical_bytes)
}

pub(super) struct HistoryDifference<S = TieredSnapshot> {
    before: Side<S>,
    after: Side<S>,
    failed: bool,
    pub(super) references_examined: u64,
    pub(super) datoms_examined: u64,
}
...
impl HistorySource for TieredSnapshot {
    fn directory(
        &self,
        reference: &ChildRef,
        reads: &mut TreeReadStats,
    ) -> Result<LoadedDirectory, SemanticError> {
        self.load_directory(reference, IndexOrder::Eavt, true, reads)
    }
    fn leaf(
        &self,
        reference: &ChildRef,
        ...
        self.load_leaf(reference, IndexOrder::Eavt, true, reads)
    }
}

impl Side<TieredSnapshot> {
    fn new(snapshot: TieredSnapshot) -> Result<Self, SemanticError> {
        let (base, root) = snapshot.exact_tree(true, IndexOrder::Eavt)?;
        if base.manifest.basis_t != snapshot.basis_t() {
            return Err(fault(
                "fulltext/noncanonical-delta-source",
                "fulltext differences require exact published sources without a recent tail",
            ));
        }
        Ok(Self::from_root(snapshot, root))
    }
}
...
impl HistoryDifference {
    pub(super) fn new(
        before: TieredSnapshot,
        after: TieredSnapshot,
    ) -> Result<Self, SemanticError> {
        let equal_roots = before
            .state
            .tree_base
            .as_ref()
            .and_then(|base| base.manifest.tree(IndexOrder::Eavt, true))
            .zip(
                after
                    .state
                    .tree_base
            ...
            .is_some_and(|(before, after)| {
                before.descriptor.root_hash == after.descriptor.root_hash
            });
        let mut difference = Self {
            before: Side::new(before)?,
        ...
        if equal_roots {
            difference.before.directory_index = difference.before.root.directories.len();
            difference.after.directory_index = difference.after.root.directories.len();
        }
        Ok(difference)
    }

    pub(super) fn source_nodes_read(&self) -> u64 {
        [&self.before, &self.after]
            .into_iter()
            .map(|side| {
                let stats = side.source.load_stats();
                stats.root_reads + stats.directory_reads + stats.leaf_reads
            })
            .sum()
    }

    pub(super) fn source_node_bytes(&self) -> u64 {
        [&self.before, &self.after]
            .into_iter()
            .map(|side| source_node_bytes(&side.source))
            .sum()
    }
}
