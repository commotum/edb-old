...
use super::{PeerIndexCursor, PeerLoadStats, lock};
use crate::persistent_tree::{TreeRangeResult, TreeSeekResult};
use crate::recent::{RecentLimits, RecentStats};
...
use crate::{
    ...
    QueryControl, QueryExtensions, QueryInput, QueryOutcome, QueryValue, Schema, SemanticError,
...
impl Peer {
    ...
    fn open(
        ...
        let id = crate::storage::engine::identity_string(database.identity);
...
impl PeerSnapshot {
    ...
    pub fn last_tx_instant(&self) -> Result<Option<i64>, SemanticError> {
        Ok(self.native.last_tx_instant())
    }
...
fn parse_identity(text: &str) -> Result<[u8; 16], SemanticError> {
    ...
    let bytes = u128::from_str_radix(&compact, 16)
        ...
        .to_be_bytes();
    if bytes == [0; 16] || crate::storage::engine::identity_string(bytes) != text {
        return Err(identity_error());
    }
    Ok(bytes)
}
