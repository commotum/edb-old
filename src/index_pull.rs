...
impl DatabaseValue {
    ...
    pub fn index_pull_with_control(
        ...
        // Existing pull-many validates even an empty input, without resolving
        // or pulling entities. Reuse that validation rather than duplicating
        // selector semantics in this separate traversal API.
        self.pull_many(&options.selector, std::iter::empty::<u64>())?;
...
impl Iterator for IndexPullCursor<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        loop {
            ...
            let datom = match self.cursor.as_mut()?.next() {
                Some(Ok(datom)) => datom,
