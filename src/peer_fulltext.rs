...
use super::*;
use crate::fulltext_store::{load_page, load_projection};
use crate::{
...
impl PostgresIndexer {
    ...
    pub fn rebuild_fulltext(&mut self) -> Result<Option<FulltextProjection>, SemanticError> {
        ...
        self.ensure_fulltext_projection(revision, hash)
    }
    pub(super) fn ensure_fulltext_projection(
        ...
        let records = crate::fulltext::fulltext_records(&db)?;
        let (projection, stats) = store.build(
            hash,
            snapshot.basis_t(),
