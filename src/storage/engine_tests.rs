...
fn applied(outcome: super::super::CasOutcome) -> Reference {
    match outcome {
        super::super::CasOutcome::Applied(reference) => reference,
...
fn reclaimed_prepared_index_is_a_gc_conflict_and_can_be_prepared_again() {
    use super::super::ownership::{BlockCollector, CollectionPhase};
    let Some(f) = fixture() else {
        return;
    };
    let mut writer =
