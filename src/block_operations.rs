//! Administrative inspection over one pinned block publication. Explicit deep
...
pub(super) fn inspect(
    ...
    let capture = reader.pin_current_reference(&format!("databases/{}", entry.database_id))?;
    let result = (|| {
        ...
        let checked = (|| {
            ...
            let metadata =
                SnapshotMetadata::decode(&metadata_id, &required(&mut store, metadata_id)?)?;
            let indexes_id = root
                .indexes
...
pub(super) fn collect(
    ...
    let (authorization, report_handoffs) = if progress.phase
        == crate::storage::ownership::CollectionPhase::Complete
    {
        control.check()?;
        let handoffs = collector.prune_report_handoffs_with_control(32, &mut || control.check())?;
        (
