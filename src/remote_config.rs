...
fn error(code:&'static str)->SemanticError {
    SemanticError::new(ErrorCategory::Incorrect,code,"invalid or unavailable remote credential configuration")
}
fn required(name:&str)->Result<String,SemanticError> {
    std::env::var(name).ok().filter(|s|!s.is_empty()).ok_or_else(||error("config/remote-environment"))
}
fn read_file(path:&str,secret:bool,limit:usize)->Result<Vec<u8>,SemanticError> {
    ...
    file.take((limit+1) as u64).read_to_end(&mut bytes).map_err(|_|error("config/remote-file"))?;
    if bytes.len()>limit {return Err(error("config/remote-file"));}
    Ok(bytes)
}
...
pub fn remote_server_credentials_from_env()->Result<(native_tls::Identity,RemoteAuthToken),SemanticError> {
    ...
    let identity=native_tls::Identity::from_pkcs8(&cert,&key).map_err(|_|error("config/remote-tls-identity"))?;
    Ok((identity,token()?))
}
...
pub fn remote_client_config_from_env()->Result<RemoteClientConfig,SemanticError> {
    ...
    match std::env::var("ATOMIC_REMOTE_TLS_ROOT") {
        Ok(path) if !path.is_empty()=>{
            ...
            config=config.with_root_certificate_pem(&root).map_err(|_|error("config/remote-tls-root"))?;
        }
...
mod tests {
    ...
    fn credential_files_are_bounded_owned_regular_private_and_redacted() {
        ...
        assert_eq!(read_file(path,true,128).unwrap(),b"secret-credential-marker");
