...
use crate::index::recent::NoHistoryPair;
...
/// Some variable values also retain an order-preserving prefix representation.
/// 1.0.7705 specializes String/vector; native byte and named-value prefixes
/// preserve their respective lexical order. URI components do not sort by raw
/// text, so URI separators retain the exact Value, including inside tuples.
#[derive(Clone, Debug)]
...
impl RoutingValue {
    ...
    fn retained_heap_bytes(&self) -> u64 {
        ...
        match self {
            ...
            Self::String(value) => u16_bytes(value),
...
fn compare_routing_values(left: &RoutingValue, right: &RoutingValue, stored: bool) -> Ordering {
    ...
    match (left, right) {
        ...
        (RoutingValue::String(left), RoutingValue::String(right)) => left.cmp(right),
        (
            RoutingValue::Bytes {
                total_len: left_len,
                prefix: left,
...
fn minimum_difference(prior: &Value, current: &Value) -> RoutingValue {
    let candidate = match (prior, current) {
        ...
        // Source mindiff only shortens strings/vectors. A raw URI prefix does
        // not preserve component order: :9 < :10 but the raw prefix :1 < :9.
        // Keeping the entire URI also repairs recursive tuple separators.
        (Value::Uri(_), Value::Uri(_)) => RoutingValue::exact(current),
...
fn decode_routing_value(
    ...
    match cursor.u8()? {
        ...
        // Tag 2 was the obsolete raw URI-prefix representation. Format 5
        // accepts URI values only through Exact's canonical value encoding.
...
mod tests {
    ...
    #[test]
    fn compact_native_separators_cover_bytes_keyword_and_symbol() {
    ...
    #[test]
    fn uri_component_separators_survive_multileaf_build_seek_and_merge() {
        let texts = [
            "http://A:9/a",
            "HTTP://a:10/a",
            "http://a:11/a",
        ...
        let value = |text: &str, nested: bool| {
            let uri = Value::Uri(text.into());
            if nested {
                Value::Tuple(vec![
                    Some(Value::Tuple(vec![
                        Some(uri),
                        Some(Value::String("retained-suffix".into())),
                    ])),
                    Some(Value::Long(7)),
                ])
            } else {
        ...
        for nested in [false, true] {
            for order in [IndexOrder::Eavt, IndexOrder::Aevt, IndexOrder::Avet] {
                // Expected order is independent of routing/value comparators:
                // numeric ports 9 < 10 < 11, then host a < b ignoring case.
                let datoms: Vec<_> = texts
                    .iter()
                    .enumerate()
                    .map(|(i, text)| Datom {
                        entity: make_eid(USER_PARTITION, 1).unwrap(),
                        attribute: 7,
                        value: value(text, nested),
                        tx: t_to_tx(i as u64 + 1).unwrap(),
                        added: true,
                    })
                    .collect();
                let built = build_tree(order, false, datoms.clone(), &config).unwrap();
                validate_tree(&built.descriptor, &built.nodes).unwrap();
                assert_exact_reads(&built, &datoms);
                let root = expect_root(
                    decode_tree_node(
                ...
                .unwrap();
                assert_eq!(root.directories.len(), 2, "exercise both routing levels");
                for (expected, alias) in datoms.iter().zip(aliases) {
                    let key = Datom {
                        value: value(alias, nested),
                        ..expected.clone()
                    };
                    let actual = seek_tree(&built.descriptor, &built.nodes, &key)
                        .unwrap()
                        .datom
                        .unwrap();
                    assert_eq!(actual, *expected);
                    assert_eq!(
                        crate::canonical_datom_bytes(&actual).unwrap(),
                        crate::canonical_datom_bytes(expected).unwrap(),
                        "retain stored spelling"
                    );
                }

                let inserted = Datom {
                    value: value("http://a:12/a", nested),
                    tx: t_to_tx(5).unwrap(),
                    ..datoms[1].clone()
                };
                let edits = TreeMergeEdits {
                    removals: vec![datoms[1].clone()],
                    insertions: vec![inserted.clone()],
                    ..TreeMergeEdits::default()
                };
                let merged = merge_tree(&built.descriptor, &built.nodes, &edits, &config).unwrap();
                let nodes = overlay(&built.nodes, &merged.new_nodes);
                validate_tree(&merged.descriptor, &nodes).unwrap();
                let expected = vec![
                    datoms[0].clone(),
                    datoms[2].clone(),
                    inserted,
                ...
                assert_eq!(
                    range_tree(&merged.descriptor, &nodes, None, None)
                        .unwrap()
                        .datoms,
                    expected
                );
                for expected in &expected {
                    assert_eq!(
                        seek_tree(&merged.descriptor, &nodes, expected)
                            .unwrap()
                            .datom
                            .as_ref(),
                        Some(expected)
                ...
                // Copy-on-write must leave the original immutable root usable.
                assert_exact_reads(&built, &datoms);
            }
        }
    }
    ...
    #[test]
    fn large_uri_separators_retain_exact_values_with_explicit_parent_limits() {
        let tail = "v".repeat(32 * 1024);
        let datoms: Vec<_> = (0..8)
            .map(|index| Datom {
                entity: make_eid(USER_PARTITION, index + 1).unwrap(),
                attribute: 7,
                value: Value::Uri(format!(
        ...
        let built = build_tree(IndexOrder::Avet, false, datoms.clone(), &config).unwrap();
        validate_tree(&built.descriptor, &built.nodes).unwrap();
        assert_exact_reads(&built, &datoms);
        let root = expect_root(
            decode_tree_node(
        ...
        for directory_ref in &root.directories {
            let directory = expect_directory(
                decode_tree_node(
                    &directory_ref.hash,
                    built.nodes.get(&directory_ref.hash).unwrap(),
                )
                .unwrap(),
            )
            .unwrap();
            for reference in &directory.leaves {
                assert!(matches!(
                    reference.key.value,
                    Some(RoutingValue::Exact(Value::Uri(_)))
                ));
                assert!(
                    child_reference_encoded_len(reference).unwrap() > tail.len(),
                    "URI parent bytes are explicit, not an unsafe raw-text prefix"
                );
            }
        }
        assert_eq!(
            decode_routing_value_bytes(&[2]).unwrap_err().code,
            "tree/invalid-routing-value-tag"
        );
    }
