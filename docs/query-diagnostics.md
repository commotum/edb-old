...
This follows the useful contract in the local Datomic Query Stats reference
(row flow, bindings and execution phases) and recovered `datomic.datalog`
`sched-in-order`/`eval-clause` instrumentation. Atomic retains its own readiness
and selectivity-based optimizer instead of importing JVM clause-order rules.
