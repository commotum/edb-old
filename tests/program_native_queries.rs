...
#[test]
fn version_one_bytes_and_hash_match_independently_captured_old_library() {
    // Captured with a pre-change release rlib, not generated by this encoder.
...
fn persisted_native_query_speculates_commits_rebinds_retries_and_recovers() {
    ...
    let ProgramOutput::Transaction(forms) = ProgramRuntime
        .execute(&retained, &historical, &args, ProgramControl::default())
        .unwrap()
    else {
