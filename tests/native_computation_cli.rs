...
#[test]
fn genuine_schema_32_upgrade_fences_old_startup_and_preserves_accepted_receipt() {
    let (Ok(url), Some(old)) = (
        std::env::var("ATOMIC_POSTGRES_URL"),
        std::env::var_os("ATOMIC_PRE_COMPUTATION_BIN"),
    ) else {
        eprintln!(
            "SKIP genuine pre-computation upgrade: requires ATOMIC_POSTGRES_URL and ATOMIC_PRE_COMPUTATION_BIN"
        );
        return;
    };
    let fixture = common::PostgresFixture::new(&url, "computation_upgrade");
    ...
    assert_eq!(
        sql.query_one("SELECT max(version) FROM atomic_schema_migrations", &[])
            ...
            .get::<_, i64>(0),
        32,
        "this witness requires a genuine old schema-32 executable"
    );
    success(
        old_command(&["create", "--database", "upgrade"])
            .output()
            .unwrap(),
    );
    ...
    let request = r#"[{:person/name "Old accepted value"}]"#;
    let args = [
        "transact",
        "--database",
        "upgrade",
    ...
    let old_report = command_input(old_command(&args), request);
    writer.stop();
    cli(&fixture.connection, &["migrate"]);
    let rejection = old_command(&["status", "--database", "upgrade"])
        .output()
        .unwrap();
    assert!(!rejection.status.success());
    assert!(String::from_utf8_lossy(&rejection.stderr).contains("postgres/schema-too-new"));
    let mut writer = Server::start(&fixture.connection, "upgrade", &endpoint);
    let replay = input(&fixture.connection, &args, request);
    ...
    for name in [
        ...
        assert_eq!(field(&replay, name), field(&old_report, name));
    }
    writer.stop();
    println!(
        "COMPUTATION_UPGRADE_OK genuine_old_schema=32 new_schema={} old_startup_fenced=true exact_receipt=true",
