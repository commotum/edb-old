...
#[test]
fn coherent_forged_base_with_recomputed_hashes_cannot_replace_published_root() {
    let Some(connection) = connection() else {
        return;
    };
    let database_id = unique("coherent_current_fault");
    let (mut store, entity) = populated(&connection, &database_id, false, 3);
    let expected = store.recover(&database_id).unwrap();
    ...
    let Some(flat_row) = client
        ...
        // Native production no longer writes the flat format. Its root
        // publication is immutable; keep the remainder of this test as the
        // explicit compatibility witness when a migrated flat base exists.
        let error = client
            .execute(
    ...
    let published_hash: Vec<u8> = client
        .query_one(
            "SELECT manifest_hash FROM atomic_index_publications WHERE database_id = $1",
            &[&database_id],
        )
        .unwrap()
        .get(0);
    let mut changed_roots = Vec::new();
    for reference in &mut manifest.segments {
        let segment_payload: Vec<u8> = client
            .query_one(
            ...
            .unwrap()
            .get(0);
        let mut segment = decode_index_segment(&segment_payload).unwrap();
        let mut changed = false;
        for datom in &mut segment.datoms {
            if datom.entity == entity
                && datom.attribute == ITEM_COUNT
                && datom.value == Value::Long(3)
            {
                datom.value = Value::Long(999);
                changed = true;
            }
        }
        if changed {
            segment
                .datoms
                .sort_by(|left, right| left.cmp_in(right, segment.order));
            let rewritten = encode_index_segment(&segment).unwrap();
            let rewritten_hash = sha256(&rewritten);
            client
                .execute(
                    "INSERT INTO atomic_index_segments (segment_hash, payload) \
                     VALUES ($1, $2) ON CONFLICT (segment_hash) DO NOTHING",
                    &[&&rewritten_hash[..], &&rewritten[..]],
                )
                .unwrap();
            reference.hash = rewritten_hash;
            changed_roots.push((reference.history, reference.order));
        }
    }
    // Every current and historical root containing the fact was rewritten, so
    // the forged base is internally coherent rather than merely disagreeing
    // with a redundant order.
    assert_eq!(changed_roots.len(), 6);
    for history in [false, true] {
        for order in [IndexOrder::Eavt, IndexOrder::Aevt, IndexOrder::Avet] {
            assert!(changed_roots.contains(&(history, order)));
        }
    }
    let rewritten_manifest = encode_index_manifest(&manifest).unwrap();
    let rewritten_manifest_hash = sha256(&rewritten_manifest);
    client
        .batch_execute("ALTER TABLE atomic_index_manifests DISABLE TRIGGER ALL")
        .unwrap();
    client
        .execute(
            "UPDATE atomic_index_manifests SET manifest_hash = $1, payload = $2 \
             WHERE database_id = $3",
            &[
    ...
    let still_published: Vec<u8> = client
        .query_one(
            "SELECT manifest_hash FROM atomic_index_publications WHERE database_id = $1",
            &[&database_id],
        )
        .unwrap()
        .get(0);
    assert_eq!(still_published, published_hash);
    assert_ne!(still_published, rewritten_manifest_hash);

    let peer = Peer::connect(&connection, &database_id, 32).unwrap();
    assert_eq!(peer.durable_base_t(), 0);
    assert_current_eq(&peer.db(), &expected);

    // Even privileged corruption that also rewrites the root publication is
    // not enough: the immutable transaction row independently commits to the
    // canonical materialized state produced by the real transaction data.
    client
        .batch_execute("ALTER TABLE atomic_index_publications DISABLE TRIGGER USER")
        .unwrap();
    client
        .execute(
            "UPDATE atomic_index_publications SET manifest_hash = $1 \
             WHERE database_id = $2",
            &[&&rewritten_manifest_hash[..], &database_id],
        )
        .unwrap();
    client
        .batch_execute("ALTER TABLE atomic_index_publications ENABLE TRIGGER USER")
        .unwrap();
    let peer = Peer::connect(&connection, &database_id, 32).unwrap();
    assert_eq!(peer.durable_base_t(), 0);
    assert_current_eq(&peer.db(), &expected);
