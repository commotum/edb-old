...
(defn restore-boolean-constants [args arg-types]
  (mapv (fn [arg arg-type]
          (if (and (= "boolean" arg-type)
                   (= :const (:op arg))
                   (#{0 1} (:val arg)))
            (assoc arg :val (= 1 (:val arg)))
            arg))
        args (concat arg-types (repeat nil))))
...
(def rt-casts
  {"booleanCast" "boolean"
   ...
   "longCast" "long"
   "shortCast" "short"
   "uncheckedByteCast" "unchecked-byte"
   "uncheckedCharCast" "unchecked-char"
   "uncheckedDoubleCast" "unchecked-double"
   "uncheckedFloatCast" "unchecked-float"
   "uncheckedIntCast" "unchecked-int"
   "uncheckedLongCast" "unchecked-long"
   "uncheckedShortCast" "unchecked-short"})

(defn restore-primitive-cast [arg arg-type preserve-constant?]
  (if-let [cast-name (get primitive-casts arg-type)]
    (if (or (and preserve-constant? (= :const (:op arg)))
            (and (= :invoke (:op arg))
                 (= "clojure.core" (get-in arg [:fn :ns]))
                 (contains? #{cast-name (str "unchecked-" cast-name)}
                            (get-in arg [:fn :name]))))
      arg
      {:op :invoke
       :fn {:op :var :ns "clojure.core" :name cast-name}
       :args [arg]})
    arg))

(defn restore-primitive-casts [args arg-types]
  (mapv #(restore-primitive-cast %1 %2 true)
        args (concat arg-types (repeat nil))))
...
(defmethod ast->sugared-ast* :set! [{:keys [target-type] :as ast}]
  (let [val (ast->sugared-ast* (:val ast))
        val (-> [val]
                (restore-boolean-constants [target-type])
                first
                (restore-primitive-cast target-type false))]
    (-> ast
...
(defmethod ast->sugared-ast* :new [{:keys [arg-types] :as ast}]
  (-> ast
      (update :args #(-> (mapv ast->sugared-ast* %)
                         (restore-boolean-constants arg-types)
                         (restore-primitive-casts arg-types)))))
...
(defmethod ast->sugared-ast* :invoke-instance [{:keys [method target-class arg-types] :as ast}]
  (let [{:keys [target args] :as ast} (-> ast
   ...
   (cond
     ...
     {:op :invoke
      :fn target
      ;; IFn's 21-argument overload stores arguments 21+ in an Object[].
      ;; Reconstruct the source-level call by splicing that array back out.
      :args (if (and (= "invoke" method)
                     (= 21 (count args))
                     (= 21 (count arg-types))
                     (every? #{"java.lang.Object"} (butlast arg-types))
                     (= "java.lang.Object[]" (last arg-types))
                     (= :array (:op (last args))))
              (into (vec (butlast args)) (deref (:!items (last args))))
              args)}
     ...
     :else
     (update ast :args #(restore-primitive-casts % arg-types)))))
...
(defmethod ast->sugared-ast* :invoke-static [{:keys [^String target method arg-types] :as ast}]
  (let [{:keys [args] :as ast} (update ast :args #(-> (mapv ast->sugared-ast* %)
    ...
    (cond
      ...
      (and (= target "clojure.lang.RT")
           (contains? rt-casts method)
           (= 1 (count args)))
      {:op :invoke
       :fn {:op :var
            :ns "clojure.core"
            :name (rt-casts method)}
       :args args}
      ...
      (and (= target "java.lang.Character")
           (= method "valueOf")
           (= 1 (count args))
           (-> args first :op (= :const))
           (-> args first :val number?))
      ...
      (let [reflective-args (vec (deref (:!items (nth args 2))))]
        {:op :invoke-instance
         :target (first args)
         :args reflective-args
         :arg-types (vec (repeat (count reflective-args) "java.lang.Object"))
