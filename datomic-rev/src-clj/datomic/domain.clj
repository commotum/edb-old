(do
     ...
     (fn fn__16773
       ([d]
         (loop [size 0 iter (.seek ^datomic.btset.BTSet d)]
           (if iter
             (recur
               (+ size (long (size/memory-size (.get ^datomic.iter.Iter iter))))
               (.next ^datomic.iter.Iter iter))
             (long size)))))})
  (extend
    datomic.db.Datum
    size/MemorySize
    {:memory-size (fn fn__16775 ([d] (+ 40 (size/memory-size (.v ^datomic.db.Datum d)))))})
  (def common-read-handlers
    ...
    ([cache_bytes]
      (do
        ...
        (cache/create-scaled-weight-limited
          ...
          (fn fn__16778 ([k v] (+ (size/memory-size k) (size/memory-size v))))
          1000)))
  ...
  (defmethod print-dup datomic.domain.ValcachePoller fn__16788 ([o w] (print-method o w)))
  (def cache-delay (delay (create-object-cache)))
        ...
        (cache/lookup-cache
          ...
          (fn fn__16795
            ([k h_or_m]
              ...
              (monitor/add-stat
                :ObjectCache
                (let [G__16796 h_or_m] (case G__16796 :miss 0 :hit 1))))))))
  ...
  (defn preload-extension-resolver!
    ([]
      (when (io/resource ext-resolver/config-resource)
        ...
        (let [logger (org.slf4j.LoggerFactory/getLogger "datomic.domain")]
          (when (.isInfoEnabled ^org.slf4j.Logger logger)
            (.info ^org.slf4j.Logger logger (logger/process {:event :extension-resolver/preload!}))
            nil)
          nil))))
  (def defressian (fressian/val->obj common-read-handlers))
  (reset-meta! #'defressian (assoc {:private true, :column 1} :name 'defressian :ns *ns*))
        ...
        (cache/lookup-cache
          ...
          (fn fn__16813
            ([k h_or_m]
              ...
              (monitor/add-stat
                :ObjectCache
                (let [G__16814 h_or_m] (case G__16814 :miss 0 :hit 1))))))))
