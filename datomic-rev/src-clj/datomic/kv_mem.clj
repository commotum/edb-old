                               ...
                               (let [oldv (.get ^java.util.Map m k)]
                                 (when (= emap (select-keys oldv (keys emap)))
                                   (or
                                     ...
                                     (recur k val emap))))))
                  map__23556 val_map
                  map__23556 (if (seq? map__23556)
                               (clojure.lang.PersistentHashMap/create (seq map__23556))
                               map__23556)
                  id (clojure.core/get map__23556 :id)
                  ensure (clojure.core/get map__23556 :ensure)
