...
| Recovered Peer Server | partial | Connection/catalog caches, Client SPI delegation, HMAC authentication, Transit codecs, bounded Nano HTTPS serving, and `/health` are source-connected ([server construction](../src-clj/datomic/peer_server.clj#L417)). Startup dereferences a packaged `datomic/transactor-key.jks` ([TLS loader](../src-clj/datomic/client_server/auth.clj#L25)), while candidate resources deliberately contain no licensed JKS or generated replacement; no retained request, authentication, failure, or shutdown run exists. |
