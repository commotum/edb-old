...
evidence_root=${2:-/tmp/datomic-stage2-cache-overlap-v1}
java_root=${DATOMIC_STAGE2_CACHE_JAVA_ROOT:-/tmp/amazon-corretto-11.0.22.7.1}
java_bin="$java_root/bin/java"
javac_bin="$java_root/bin/javac"
probe="$script_dir/stage2/cache_overlap_probe.clj"

fail() {
  echo "Stage 2 cache-overlap gate: $*" >&2
  exit 1
}
...
run_lane() {
  ...
  if (
    ...
    /usr/bin/timeout --signal=TERM --kill-after=5s 90s \
      ...
      -Dstage2.cache.probe="$probe" \
      -Dstage2.cache.expected-kind="$kind" \
      -Dstage2.cache.expected-cache-origin="$cache_origin" \
      -Dstage2.cache.expected-version-origin="$version_origin" \
      -Dstage2.cache.expected-java-origin="$java_origin" \
      -cp "$classpath" clojure.main -e \
      '(binding [*err* (java.io.StringWriter.)] (load-file (System/getProperty "stage2.cache.probe")))'
  ...
  [[ $(grep -Fc 'STAGE2_CACHE_OVERLAP_SUPPORTED_RESULT ' \
              "$lane_root/stdout") == 1 ]] ||
    fail "$lane did not emit exactly one supported-domain result"
  [[ $(grep -Fc 'STAGE2_CACHE_OVERLAP_COMPILER_DOMAIN_RESULT ' \
              "$lane_root/stdout") == 1 ]] ||
    fail "$lane did not emit exactly one compiler-domain result"
  grep -F 'STAGE2_CACHE_OVERLAP_SUPPORTED_RESULT ' \
    "$lane_root/stdout" >"$lane_root/supported.result"
  grep -F 'STAGE2_CACHE_OVERLAP_COMPILER_DOMAIN_RESULT ' \
...
for lane in recovered-peer original-transactor recovered-transactor; do
  cmp -s "$supported_reference" "$evidence_root/lanes/$lane/supported.result" ||
    fail "supported cache behavior differs between original Peer and $lane"
done
  ...
  "$evidence_root/lanes/original-transactor/provenance.result" | awk '{print $1}')
{
  ...
  printf 'supported_domain_behavior\tPASS\t%s\n' \
    'all four lanes emitted byte-identical alias, lookup, transformation, cache-stack, read-ahead, inflight, error, and cleanup results'
  printf 'peer_provenance_fidelity\tPASS\t%s\n' \
    'original and recovered Peer emitted byte-identical singleton-map and singleton-sequence behavior at all three compiler sites'
  printf 'transactor_provenance_fidelity\tPASS\t%s\n' \
    'original and recovered Transactor emitted byte-identical singleton-map and singleton-sequence behavior at all three compiler sites'
  printf 'classified_cross_artifact_divergence\tPASS\t%s\n' \
    'Peer and Transactor preserve their distinct Clojure compiler map-destructuring behavior in both transformers and repairing-cache-stack'
  printf 'candidate_origin\tPASS\t%s\n' \
    'each recovered lane proved current repository cache source, freshly compiled Java, and candidate-owned VERSION origins'
  ...
  printf 'failure_cleanup\tPASS\t%s\n' \
    'inflight failure removes the owned delay and retries; repairing close failure preserves first-failure ordering'
...
echo "STAGE2_CACHE_OVERLAP_PASS supported_sha256=$supported_sha peer_provenance_sha256=$peer_provenance_sha transactor_provenance_sha256=$transactor_provenance_sha manifest_sha256=$manifest_sha evidence_root=$evidence_root"
