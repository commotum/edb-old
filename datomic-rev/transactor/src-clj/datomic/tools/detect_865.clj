(do
  ...
  (reset-meta! #'avto-cmp (assoc {:tag java.util.Comparator, :column 1} :name 'avto-cmp :ns *ns*))
  (defn lesser-avto
      ...
      (let [vec__20091 p__20090 unique_datom_file (nth vec__20091 (int 0) nil) done (promise)]
        (try
          (let [cr (tools/connection-resources uri)
                ...
                goal_t (d/basis-t db)
                _ (tools/progress prn {:phase :detect/start, :goal-t goal_t, :uri uri, :version 1})
