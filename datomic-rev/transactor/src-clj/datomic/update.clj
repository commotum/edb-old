(do
  ...
  (let [v__6812__auto__ #'prefetch-channels-ref]
  ...
  (reset-meta!
    ...
    (assoc
      ...
      :ns
      *ns*))
  (def segment-prefetch-processor
   (fn segment_prefetch_processor
  ...
  (reset-meta!
    ...
    (assoc
      ...
      :ns
      *ns*))
  (def channel-forwarding-dispatcher
   (fn channel_forwarding_dispatcher
   ...
   (fn reader
     ([shutdown_hook & p__25012]
       (let [map__25013 p__25012
             ...
             sync_queue (aclient/fressian-producer
                          session
                          sync_producer
                          (tx/write-handlers false))]
  ...
  (reset-meta!
    ...
    (assoc
      ...
      :ns
      *ns*))
  (def process-new-index
   (fn process_new_index
     ([p__25033 p__25034]
     ...
     ([shutdown_hook & p__25086]
       (let [map__25087 p__25086
         ...
         (^clojure.lang.IFn shutdown_hook
           (fn fn__25088
             ([]
               (deref (common/async-shutdown producer))
               (deref (common/async-shutdown session)))))
  ...
  (reset-meta!
    ...
    (assoc
      ...
      :ns
      *ns*))
  (def fressian-tx-notification
   (fn fressian_tx_notification
     ([processor_result]
       (try
  ...
  (reset-meta!
    ...
    (assoc
      ...
      :ns
      *ns*))
  (def notify-fressianer
   (fn notify_fressianer
  ...
  (let [v__5792__auto__ #'writer-process]
    (when-not (and
      ...
      (.bindRoot
        ...
        (clojure.lang.MultiFn.
          "writer-process"
          (fn fn__25142 ([item ctxt] (:type item)))
          :default
  ...
  (let [v__5792__auto__ #'handle-log-tree-request]
    (when-not (and
                (.hasRoot ^clojure.lang.Var v__5792__auto__)
                (instance? clojure.lang.MultiFn (deref v__5792__auto__)))
      (.setMeta (clojure.lang.RT/var "datomic.update" "handle-log-tree-request") {:column (int 1)})
  ...
  (let [protocol_metadata__7420 {:column (int 1)}]
    ...
    (reset-meta!
      ...
      (assoc (assoc protocol_metadata__7420 :doc nil) :name 'DatabaseImpl :ns *ns*))
  ...
  (let [protocol_metadata__7423 {:column (int 1)}]
    ...
    (reset-meta!
      ...
      (assoc (assoc protocol_metadata__7423 :doc nil) :name 'IMaster :ns *ns*))
  ...
  (let [protocol_metadata__7434 {:column (int 1)}]
    ...
    (reset-meta!
      ...
      (assoc (assoc protocol_metadata__7434 :doc nil) :name 'CatalogService :ns *ns*))
  ...
  (def acquire-transactor-semaphore
   (fn acquire_transactor_semaphore
     ([sem]
  ...
  (reset-meta!
    ...
    (assoc
      ...
      :ns
      *ns*))
  (def compute-index-parallelism
   (fn compute_index_parallelism
             ...
             :else (do
                     (java.lang.Double/valueOf
                       (double
                         (*
                           max_par
                           (/ (double (- bytes threshold)) (- redline threshold))))))))))))
  (reset-meta!
    ...
    (assoc
      ...
      :ns
      *ns*))
  (def start-dynamic-index-parallelism
   (fn start_dynamic_index_parallelism
     ([memidx_usage]
       (let [threshold (config/property "datomic.memoryIndexThreshold")
             ...
             config {:threshold threshold,
                     ...
                     :max-par
                     (long
                       (max 1 (quot (.availableProcessors (java.lang.Runtime/getRuntime)) 2)))}]
         (let [logger (org.slf4j.LoggerFactory/getLogger "datomic.update")]
           (when (.isInfoEnabled ^org.slf4j.Logger logger)
             (.info
               ^org.slf4j.Logger logger
