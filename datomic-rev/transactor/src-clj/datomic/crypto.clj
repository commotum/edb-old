(do
             ...
             map__20355 (if (seq? map__20355)
                          (if (next map__20355)
                            (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                              (to-array map__20355))
                            (if (seq map__20355) (first map__20355) {}))
                          map__20355)
             key (get map__20355 :key)
             alg (get map__20355 :alg)
  ...
  (def calc-hmac
   (fn calc_hmac
     ([bbuf alg k]
       (let [mac (let [G__20360 (Mac/getInstance ^java.lang.String alg)]
                   (.init ^javax.crypto.Mac G__20360 ^java.security.Key k)
                   G__20360)
     ...
     ([input k op extra]
       (let [ciph (cipher (str (.getAlgorithm ^java.security.Key k) "/CBC/PKCS5Padding"))
         ...
         (.init
           ...
           (int
             (let [G__20365 op]
               (case G__20365 :encrypt Cipher/ENCRYPT_MODE :decrypt Cipher/DECRYPT_MODE)))
           ^java.security.Key k
   ...
   (fn keystore
     ([f password type]
       (with-open [is (jio/input-stream f)]
         (let [G__20369 (KeyStore/getInstance ^java.lang.String type)]
           (.load ^java.security.KeyStore G__20369 ^java.io.InputStream is ^chars password)
           G__20369)))
