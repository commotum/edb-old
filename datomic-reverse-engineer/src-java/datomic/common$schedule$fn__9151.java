/*
 * Decompiled with CFR 0.152.
 * 
 * Could not load the following classes:
 *  clojure.lang.AFunction
 *  clojure.lang.IFn
 *  clojure.lang.Keyword
 *  clojure.lang.RT
 *  clojure.lang.Var
 *  org.slf4j.Logger
 *  org.slf4j.LoggerFactory
 */
package datomic;

import clojure.lang.AFunction;
import clojure.lang.IFn;
import clojure.lang.Keyword;
import clojure.lang.RT;
import clojure.lang.Var;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

public final class common$schedule$fn__9151
extends AFunction {
    Object f;
    public static final Var const__0 = RT.var((String)"datomic.slf4j", (String)"process");
    public static final Var const__1 = RT.var((String)"datomic.slf4j", (String)"caused-by");
    public static final Var const__2 = RT.var((String)"datomic.monitor", (String)"alarm");
    public static final Keyword const__3 = RT.keyword(null, (String)"UnhandledException");

    public common$schedule$fn__9151(Object object) {
        this.f = object;
    }

    public Object invoke(Object object) {
        Object object2;
        try {
            object2 = ((IFn)this.f).invoke();
        }
        catch (Throwable t2) {
            Logger logger = LoggerFactory.getLogger((String)"datomic.common");
            Object t2 = null;
            Throwable ex = t2;
            if (logger.isWarnEnabled()) {
                logger.warn((String)((IFn)const__0.getRawRoot()).invoke((Object)"Scheduled task failed"), ex);
                Logger logger2 = logger;
                logger = null;
                Throwable throwable = ex;
                ex = null;
                ((IFn)const__1.getRawRoot()).invoke((Object)logger2, (Object)throwable);
            }
            object2 = ((IFn)const__2.getRawRoot()).invoke((Object)const__3);
        }
        return object2;
    }
}

