(do
  ...
  (deftype
    ...
    (release-pending-syncs
      ...
      (locking lck
       (do
         (let [G__20328 (.peek ^java.util.Queue q)
               map__20329 G__20328
               map__20329 (if (seq? map__20329)
                            (if (next map__20329)
                              (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                                (to-array map__20329))
                              (if (seq map__20329) (first map__20329) {}))
                            map__20329)
               t (get map__20329 :t)
               prom (get map__20329 :prom)]
           (loop [G__20328 G__20328]
             (let [map__20330 G__20328
                   map__20330 (if (seq? map__20330)
                                (if (next map__20330)
                                  (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                                    (to-array map__20330))
                                  (if (seq map__20330) (first map__20330) {}))
                                map__20330)
                   t (get map__20330 :t)
                   prom (get map__20330 :prom)]
  ...
  (defn peer-queue-exceeded
    ([]
      (let [msg__8462__auto__ "Peer work queue limit exceeded, transactor may be unavailable."]
        (clojure.lang.ExceptionInfo.
          (str :db.error/peer-queue-exceeded " " msg__8462__auto__)
          (error/anomalize
  ...
  (deftype
          ...
          ([]
            (try
              (let [idxroot (index/find-index-root-id cluster)
                ...
                (if temp__5802__auto__
                  (let [map__20428 temp__5802__auto__
                        map__20428 (if (seq? map__20428)
                                     (if (next map__20428)
                                       (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                                         (to-array map__20428))
                                       (if (seq map__20428) (first map__20428) {}))
                                     map__20428)
                        basis_db (get map__20428 :basis-db)
                        adopted_db (get map__20428 :adopted-db)
                        msec (get map__20428 :msec)
                        iterations (get map__20428 :iterations)]
    ...
    (notify-data
      [this msg]
      (let [map__20419 msg
            map__20419 (if (seq? map__20419)
                         (if (next map__20419)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__20419))
                           (if (seq map__20419) (first map__20419) {}))
                         map__20419)
            id (get map__20419 :id)
            data (get map__20419 :data)
            tempids (get map__20419 :tempids)
            io_stats (get map__20419 :io-stats)
            start (java.lang.System/currentTimeMillis)
            prom (cache/remove pending_txes id)
            map__20420 (meta prom)
            map__20420 (if (seq? map__20420)
                         (if (next map__20420)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__20420))
                           (if (seq map__20420) (first map__20420) {}))
                         map__20420)
            io_context (get map__20420 :io-context)
            old_db (deref db_ref)
            new_db (let [m_20421 {:event :peer/accept-new, :id id}
                         ___8552__auto__ (let [logger (org.slf4j.LoggerFactory/getLogger
                                                        "datomic.peer")]
                                           (when (.isDebugEnabled ^org.slf4j.Logger logger)
                                             (.debug
                                               ^org.slf4j.Logger logger
                                               (logger/process (assoc m_20421 :phase :begin))))
                                           nil)
                         ...
                         elapsed_20422 (- (java.lang.System/nanoTime) start__8553__auto__)
                         msec_20423 (logger/format-as-msec (long elapsed_20422))]
                     (monitor/add-stat :PeerAcceptNewMsec msec_20423)
                     (let [endmsg__8556__auto__ (merge
                                                  (assoc m_20421 :msec msec_20423 :phase :end)
    ...
    (^void release
      ...
      (do
        (locking connection-lock
         (loop [seq_20409 (seq (cache/cache-keys connection-cache))
                chunk_20410 nil
                count_20411 0
                i_20412 0]
           (if (< i_20412 count_20411)
             (let [k (.nth ^clojure.lang.Indexed chunk_20410 (int i_20412))]
               (when (= this (get connection-cache k)) (cache/remove connection-cache k))
               (recur seq_20409 chunk_20410 count_20411 (inc i_20412)))
             (let [temp__5804__auto__ (seq seq_20409)]
               (when temp__5804__auto__
                 (let [seq_20409 temp__5804__auto__]
                   (if (chunked-seq? seq_20409)
                     (let [c__6065__auto__ (chunk-first seq_20409)]
                       (recur
                         (chunk-rest seq_20409)
                         c__6065__auto__
                         (int (count c__6065__auto__))
                         (int 0)))
                     (let [k (first seq_20409)]
                       (when (= this (get connection-cache k)) (cache/remove connection-cache k))
                       (recur (next seq_20409) nil 0 0)))))))))
    ...
    (create-connection-state
      ...
      (do
        (let [logger (org.slf4j.LoggerFactory/getLogger "datomic.peer")]
          ...
          nil)
        (let [m_20372 {:event :peer/create-connection-impl,
              ...
              ___8552__auto__ (let [logger (org.slf4j.LoggerFactory/getLogger "datomic.peer")]
                                (when (.isDebugEnabled ^org.slf4j.Logger logger)
                                  (.debug
                                    ^org.slf4j.Logger logger
                                    (logger/process (assoc m_20372 :phase :begin))))
                                nil)
              start__8553__auto__ (java.lang.System/nanoTime)
              result__8554__auto__ (try
                                     {:returned
                                                      ...
                                                      (do
                                                        (try
                                                          ...
                                                          (catch
                                                            ...
                                                            (error/report t__8509__auto__)))
                                                        (throw
                                                          ^java.lang.Throwable t__8510__auto__)
                                                        nil))))
                                                (catch
                                                  java.lang.Throwable
                                                  t__8510__auto__
                                     ...
                                     (catch
                                       ...
                                       t__8555__auto__
                                       {:threw t__8555__auto__}))
              elapsed_20373 (- (java.lang.System/nanoTime) start__8553__auto__)
              msec_20374 (logger/format-as-msec (long elapsed_20373))]
          (let [endmsg__8556__auto__ (merge
                                       (assoc m_20372 :msec msec_20374 :phase :end)
                                       (when (:threw result__8554__auto__)
    ...
    (^java.util.concurrent.BlockingQueue txReportQueue
      ...
      (or
        ...
        (swap!
          ...
          (fn fn__20485
            ([p1__20481#]
              (if p1__20481# p1__20481# (java.util.concurrent.LinkedBlockingQueue.)))))))
    (^datomic.ListenableFuture transactAsync
      [this ^java.util.List txdata]
    ...
    ([p__20522]
      (let [map__20523 p__20522
            map__20523 (if (seq? map__20523)
                         ...
                         map__20523)
            resolved-cluster-conf map__20523
            db-id (get map__20523 :db-id)]
        (when-not :db-id
          (throw (java.lang.AssertionError. (str "Assert failed: " (pr-str :db-id)))))
        (let [temp__5804__auto__ (get connection-cache resolved-cluster-conf)]
          (when temp__5804__auto__
  ...
  (defn get-connection
    ([cluster-conf]
      (let [m_20526 {:event :peer/get-connection,
            ...
            result__8554__auto__ (try
                                   {:returned
                                    (let [temp__5802__auto__ (coord/resolve-db-name cluster-conf)]
                                      (if temp__5802__auto__
                                              ...
                                              map__20530 (if (seq? map__20530)
                                                           (if
                                                             ...
                                                             (if
                                                               ...
                                                               {}))
                                                           map__20530)
                                              resolved-cluster-conf map__20530
                                              db-id (get map__20530 :db-id)]
                                          (locking connection-lock
  ...
  (defn connect-to-backup
    ([backup-uri t]
      (let [map__20543 (req/require-and-run 'datomic.backup/load-database backup-uri t)
  ...
  (defn connect-to-storage
    ([cluster-conf]
      (let [map__20545 (coord/resolve-db-name cluster-conf)
            map__20545 (if (seq? map__20545)
                         ...
                         map__20545)
            resolved-conf map__20545
            db-id (get map__20545 :db-id)
            cluster (coord/create-db-cluster resolved-conf)
            olookup (domain/system-cache-olookup cluster)
            map__20546 (db-io/load-db-from-basis cluster olookup db-id nil true)
  ...
  (reset-meta!
    ...
    (assoc
      ...
      :ns
      *ns*))
  (defmethod pprint/simple-dispatch datomic.db.Db fn__20567 ([x] (pr x)))
        ...
        (when temp__5804__auto__
          (let [txes temp__5804__auto__]
            (let [logger (org.slf4j.LoggerFactory/getLogger "datomic.peer")]
              ...
              nil)
            (loop [seq_20572 (seq txes) chunk_20573 nil count_20574 0 i_20575 0]
              (if (< i_20575 count_20574)
                (let [tx (.nth ^clojure.lang.Indexed chunk_20573 (int i_20575))]
                  (transact conn tx)
                  (recur seq_20572 chunk_20573 count_20574 (inc i_20575)))
                (let [temp__5804__auto__ (seq seq_20572)]
                  (when temp__5804__auto__
                    (let [seq_20572 temp__5804__auto__]
                      (if (chunked-seq? seq_20572)
                        (let [c__6065__auto__ (chunk-first seq_20572)]
                          (recur
                            (chunk-rest seq_20572)
                            c__6065__auto__
                            (int (count c__6065__auto__))
                            (int 0)))
                        (let [tx (first seq_20572)]
                          (transact conn tx)
                          (recur (next seq_20572) nil 0 0)))))))))))))
  (reset-meta!
    #'ensure-schema-level
    (assoc
    ...
    ([uri]
      (let [cluster-conf (uri/parse-db uri)
        ...
        (if (= protocol :mem)
          ...
          (let [temp__5802__auto__ (coord/resolve-db-name cluster-conf)]
            (if temp__5802__auto__
              (let [map__20598 temp__5802__auto__
                    map__20598 (if (seq? map__20598)
                                 (if (next map__20598)
                                   (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                                     (to-array map__20598))
                                   (if (seq map__20598) (first map__20598) {}))
                                 map__20598)
                    resolved-cluster-conf map__20598
                    db-id (get map__20598 :db-id)]
                (locking connection-lock
                 (do
