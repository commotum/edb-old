(do
  ...
  (.bindRoot
    ...
    (serialized (fn fn__20843 ([report data] (^clojure.lang.IFn report data)))))
  ...
  (defn set-index-basis
    ([uri basisT]
      (let [map__20848 (connection-resources uri)
            map__20848 (if (seq? map__20848)
                         (if (next map__20848)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__20848))
                           (if (seq map__20848) (first map__20848) {}))
                         map__20848)
            cr map__20848
            cluster (get map__20848 :cluster)
            olookup (get map__20848 :olookup)
            map__20849 (db-resources cr)
            map__20849 (if (seq? map__20849)
                         (if (next map__20849)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__20849))
                           (if (seq map__20849) (first map__20849) {}))
                         map__20849)
            db (get map__20849 :db)
            log (get map__20849 :log)
  ...
  (defn log-size*
    ([conn t]
      (let [log (d/log conn) db (d/db conn)]
        (reduce
          (fn fn__20869 ([size logentry] (+ size (size/memory-size logentry))))
          0
  ...
  (defn unique-identities
    ([db]
      (mapv
        :id
        (filter
          (fn fn__20883 ([p1__20882#] (= :db.unique/identity (:unique p1__20882#))))
          (map
            (fn fn__20885 ([p1__20881#] (d/attribute db (:v p1__20881#))))
  ...
  (defn read-capacity-units
    ([uri]
      (let [map__20888 (uri/parse uri)
            map__20888 (if (seq? map__20888)
                         (if (next map__20888)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__20888))
                           (if (seq map__20888) (first map__20888) {}))
                         map__20888)
            parsed_uri map__20888
            protocol (get map__20888 :protocol)]
        (cond
  ...
  (defn storage-diagnostics
    ([uri]
      (let [map__20899 (connection-resources uri)
            map__20899 (if (seq? map__20899)
                         (if (next map__20899)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__20899))
                           (if (seq map__20899) (first map__20899) {}))
                         map__20899)
            cr map__20899
            cluster (get map__20899 :cluster)
            olookup (get map__20899 :olookup)
            vec__20900 (log/read-tail-descriptor cluster)
            desc (nth vec__20900 (int 0) nil)
            buf (nth vec__20900 (int 1) nil)
  ...
  (defn unique-values
    ([db]
      (mapv
        :id
        (filter
          (fn fn__20930 ([p1__20929#] (= :db.unique/value (:unique p1__20929#))))
          (map
            (fn fn__20932 ([p1__20928#] (d/attribute db (:v p1__20928#))))
            (d/datoms db :aevt :db.install/attribute))))))
  (reset-meta!
    #'unique-values
  ...
  (defn card-ones
    ([db]
      (mapv
        :id
        (filter
          (fn fn__20937 ([p1__20936#] (= :db.cardinality/one (:cardinality p1__20936#))))
          (map
            (fn fn__20939 ([p1__20935#] (d/attribute db (:v p1__20935#))))
            (d/datoms db :aevt :db.install/attribute))))))
  (reset-meta!
    #'card-ones
    ...
    ([db]
      (mapv
        ...
        (filter
          :fulltext
          (map
            (fn fn__20943 ([p1__20942#] (d/attribute db (:v p1__20942#))))
            (d/datoms db :aevt :db.install/attribute))))))
  (reset-meta!
    #'fulltexts
  ...
  (defn avof-map
    ([datoms]
      (reduce (fn fn__20959 ([m d] (update-in m [(db/->AVof d)] (fnil conj []) d))) {} datoms)))
  (reset-meta!
    #'avof-map
    ...
    ([log ts ids]
      (avof-map
        (filter
          (fn fn__20964
            ([p__20963]
              (let [map__20965 p__20963
                    map__20965 (if (seq? map__20965)
                                 (if (next map__20965)
                                   (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                                     (to-array map__20965))
                                   (if (seq map__20965) (first map__20965) {}))
                                 map__20965)
                    a (get map__20965 :a)]
                (contains? ids a))))
          (mapcat
            (fn fn__20967
              ([p1__20962#]
                (let [tx (iter/iget (log/seek-tx log p1__20962#))]
                  (when (= p1__20962# (:t tx)) (:data tx)))))
            ts)))))
  (reset-meta!
    #'uniques-in-ts
    (assoc
      ...
      (seq
        (map
          ...
          (remove
            (fn fn__20973
              ([p__20972]
                (let [vec__20974 p__20972
                      a (nth vec__20974 (int 0) nil)
                      b (nth vec__20974 (int 1) nil)]
  ...
  (defn nohistory-checker
    ([db]
      (let [attrs (ever-nohistory-attrs db)]
        (fn fn__20980 ([d] (contains? attrs (.a ^datomic.Datom d)))))))
  (reset-meta!
    #'nohistory-checker
    (assoc
      {:arglists (clojure.core/list ['db]), :column (int 1)}
