(do
  ...
  (extend
    ...
    TreeWalker
    {:child-node-ids (fn fn__18028 ([root] (map str (vals (:attrmap root))))),
     :subtrees
     (fn fn__18030 ([root ids_>nodes] (^clojure.lang.IFn ids_>nodes (child-node-ids root))))})
  (extend
    datomic.clusterfs.ClusterFS
    TreeWalker
    {:child-node-ids (fn fn__18032 ([clusterfs] (clusterfs/all-keys clusterfs))),
     :subtrees (fn fn__18034 ([_ _] nil))})
  (extend
    datomic.index.RootNode
    TreeWalker
    {:child-node-ids (fn fn__18036 ([rn] (map str (.-dirids ^datomic.index.RootNode rn)))),
     :subtrees
     (fn fn__18038 ([rn ids_>nodes] (^clojure.lang.IFn ids_>nodes (child-node-ids rn))))})
  (extend
    datomic.index.DirNode
    TreeWalker
    {:child-node-ids (fn fn__18040 ([dn] (map str (.-segids ^datomic.index.DirNode dn)))),
     :subtrees (fn fn__18042 ([_ _] nil))})
  (deftype
    Node
    [id walker]
  ...
  (defn create-node
    ([id lookup allow_missing?]
      ...
      (let [temp__5825__auto__ (lookup-val lookup id allow_missing?)]
        (when temp__5825__auto__ (let [v temp__5825__auto__] (->Node id v))))))
  (reset-meta!
    #'create-node
    (assoc
      {:arglists
        ...
        (fn fn__18053
          ([ids]
            (remove
              nil?
              (map
                (fn fn__18054 ([p1__18052#] (create-node p1__18052# lookup allow_missing?)))
                ids))))))
  ...
  (defn create-parent-node
    ([id walker lookup allow_missing?]
      (let [ids_>nodes (create-ids->nodes lookup allow_missing?)
            temp__5825__auto__ (lookup-val lookup id allow_missing?)]
        (when temp__5825__auto__
          (let [top temp__5825__auto__]
  ...
  (defn tree-node-ids
    ([node ids_>nodes]
      (concat
        (mapcat
          (fn fn__18061 ([p1__18060#] (tree-node-ids p1__18060# ids_>nodes)))
      ...
      (reify
        ...
        (subtrees
          ...
          (map
            (fn fn__18071
              ([p__18070]
                (let [map__18072 p__18070
                      map__18072 (if (seq? map__18072)
                                   (if (next map__18072)
                                     (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                                       (to-array map__18072))
                                     (if (seq map__18072) (first map__18072) {}))
                                   map__18072)
                      uuid (get map__18072 :uuid)
                      temp__5825__auto__ (lookup-val lookup (str uuid) allow_missing?)]
                  (when temp__5825__auto__
