(do
  ...
  (def get-bucket
   (fn get_bucket
     ([s3 bucket_name]
       (first
         (filter
           (fn fn__26698 ([b] (= (:Name b) bucket_name)))
  ...
  (defn get-non-direct-buffer
    ([s3 bucket path]
      (let [temp__5825__auto__ (:body (get-object s3 bucket path :bytes))]
        (when temp__5825__auto__ (let [b temp__5825__auto__] (ByteBuffer/wrap ^bytes b))))))
  (reset-meta!
  ...
  (defn get-file
    ([s3 bucket path dest]
      (let [temp__5825__auto__ (get-object s3 (s3-name bucket) (s3-name path) :input-stream)]
