(do
    ...
    (when-not (and
      ...
      (.bindRoot
        ...
        (clojure.lang.MultiFn.
          ...
          (fn fn__27678
            ([p__27677 sc]
              (let [map__27679 p__27677
                    map__27679 (if (seq? map__27679)
                                 (if (next map__27679)
                                   (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                                     (to-array map__27679))
                                   (if (seq map__27679) (first map__27679) {}))
                                 map__27679)
                    m map__27679
                    opcode (get map__27679 :opcode)]
                opcode)))
          :default
          #'clojure.core/global-hierarchy))
      #'handle))
  (.setMeta (clojure.lang.RT/var "datomic.valcache" "sasl") {:column (int 1)})
  (let [v__5813__auto__ #'sasl]
    (when-not (and
                (.hasRoot ^clojure.lang.Var v__5813__auto__)
                (instance? clojure.lang.MultiFn (deref v__5813__auto__)))
      (.setMeta (clojure.lang.RT/var "datomic.valcache" "sasl") {:column (int 1)})
      (.bindRoot
        ...
        (clojure.lang.MultiFn.
          ...
          (fn fn__27686
            ([p__27685 sc]
              (let [map__27687 p__27685
                    map__27687 (if (seq? map__27687)
                                 (if (next map__27687)
                                   (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                                     (to-array map__27687))
                                   (if (seq map__27687) (first map__27687) {}))
                                 map__27687)
                    m map__27687
                    opcode (get map__27687 :opcode)]
                opcode)))
          :default
          #'clojure.core/global-hierarchy))
      #'sasl))
  ...
  (def supported-or-drain
   (fn supported_or_drain
     ([p__27693 sc mmeth]
       (let [map__27694 p__27693
             map__27694 (if (seq? map__27694)
                          (if (next map__27694)
                            (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                              (to-array map__27694))
                            (if (seq map__27694) (first map__27694) {}))
                          map__27694)
             header map__27694
             opcode (get map__27694 :opcode)
             data_type (get map__27694 :data-type)
             vbucket_id (get map__27694 :vbucket-id)
             cas (get map__27694 :cas)
             total_body_length (get map__27694 :total-body-length)]
         (if (not
               (and
                 (zero? data_type)
                 (zero? vbucket_id)
  ...
  (def noop-quit
   (fn noop_quit
     ([p__27700 sc]
       (let [map__27701 p__27700
             map__27701 (if (seq? map__27701)
                          (if (next map__27701)
                            (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                              (to-array map__27701))
                            (if (seq map__27701) (first map__27701) {}))
                          map__27701)
             opcode (get map__27701 :opcode)
             key_length (get map__27701 :key-length)
             extras_length (get map__27701 :extras-length)
             total_body_length (get map__27701 :total-body-length)]
  ...
  (reset-meta!
    ...
    (assoc
      ...
      :ns
      *ns*))
  (defmethod handle 10 fn__27705 ([header sc] (noop-quit header sc)))
  (defmethod handle 7 fn__27707 ([header sc] (noop-quit header sc)))
  (defn uuid-prefix
    ([s]
      (try
  ...
  (def full-path
   (fn full_path
     ([root k]
       (let [temp__5825__auto__ (uuid-prefix k)]
         (when temp__5825__auto__
           (let [uuid_prefix temp__5825__auto__]
             (.getPath
  ...
  (defmethod
    ...
    fn__27713
    ([p__27712 sc]
      (let [map__27714 p__27712
            map__27714 (if (seq? map__27714)
                         (if (next map__27714)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__27714))
                           (if (seq map__27714) (first map__27714) {}))
                         map__27714)
            header map__27714
            opcode (get map__27714 :opcode)
            key_length (get map__27714 :key-length)
            extras_length (get map__27714 :extras-length)
            total_body_length (get map__27714 :total-body-length)
            root (get map__27714 :root)]
        (if (or
              (zero? key_length)
              (not= extras_length 8)
  ...
  (defmethod
    ...
    fn__27723
    ([p__27722 sc]
      (let [map__27724 p__27722
            map__27724 (if (seq? map__27724)
                         (if (next map__27724)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__27724))
                           (if (seq map__27724) (first map__27724) {}))
                         map__27724)
            opcode (get map__27724 :opcode)
            key_length (get map__27724 :key-length)
            extras_length (get map__27724 :extras-length)
            total_body_length (get map__27724 :total-body-length)
            root (get map__27724 :root)]
        (if (not
              (and
  ...
  (defmethod
    ...
    fn__27730
    ([p__27729 sc]
      (let [map__27731 p__27729
            map__27731 (if (seq? map__27731)
                         (if (next map__27731)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__27731))
                           (if (seq map__27731) (first map__27731) {}))
                         map__27731)
            opcode (get map__27731 :opcode)
            key_length (get map__27731 :key-length)
            extras_length (get map__27731 :extras-length)
            total_body_length (get map__27731 :total-body-length)
            root (get map__27731 :root)]
        (if (not
              (and
                (clojure.lang.Numbers/isPos key_length)
                (zero? extras_length)
  ...
  (defmethod
    ...
    fn__27739
    ([p__27738 sc]
      (let [map__27740 p__27738
            map__27740 (if (seq? map__27740)
                         (if (next map__27740)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__27740))
                           (if (seq map__27740) (first map__27740) {}))
                         map__27740)
            m map__27740
            opcode (get map__27740 :opcode)
            key_length (get map__27740 :key-length)
            extras_length (get map__27740 :extras-length)
            total_body_length (get map__27740 :total-body-length)
            root (get map__27740 :root)
            sasl (get map__27740 :sasl)]
        (if (not (clojure.lang.Numbers/isPos key_length))
          ...
          (let [k (read-key key_length sc)
            ...
            (if (= k "PLAIN")
              (let [vec__27741 (scan-strings bb)
                    username (nth vec__27741 (int 0) nil)
                    password (nth vec__27741 (int 1) nil)]
                        ...
                        (let [threshold (/ threshold 4096)
                              ...
                              pq (java.util.PriorityQueue.
                                   ...
                                   (comparator
                                     (fn fn__27812
                                       ([p1__27809# p2__27810#]
                                         (> (:atime p1__27809#) (:atime p2__27810#))))))
                              size (atom 0)
                              visited (atom 0)]
                          (Files/walkFileTree
                            ^java.nio.file.Path path
                              ...
                              (loop [i (dec (count fa)) deleted 0 files 0]
                                (if (or (neg? i) (>= deleted target))
                                  (let [result {:deleted-files (long files),
                                    ...
                                    result)
                                  (let [map__27816 (aget ^"[Ljava.lang.Object;" fa (int i))
                                        map__27816 (if (seq? map__27816)
                                                     (if (next map__27816)
                                                       (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                                                         (to-array map__27816))
                                                       (if (seq map__27816) (first map__27816) {}))
                                                     map__27816)
                                        atime (get map__27816 :atime)
                                        length (get map__27816 :length)
                                        file (get map__27816 :file)]
  ...
  (defn shutdown
    ([]
      (let [temp__5825__auto__ (deref valcache-ref)]
        (when temp__5825__auto__
          (let [valcache temp__5825__auto__] (compare-and-set! valcache-ref valcache nil))))))
  (reset-meta!
    #'shutdown
   ...
   (fn sasl_loop
     ([creds sc sem]
       (loop [hb (ByteBuffer/wrap (byte-array 24))]
         (let [map__27828 (read-header sc hb)
               map__27828 (if (seq? map__27828)
                            (if (next map__27828)
                              (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                                (to-array map__27828))
                              (if (seq map__27828) (first map__27828) {}))
                            map__27828)
               header map__27828
               opcode (get map__27828 :opcode)]
   ...
   (fn start_server
     ([p__27834]
       (let [map__27835 p__27834
             map__27835 (if (seq? map__27835)
                          (if (next map__27835)
                            (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                              (to-array map__27835))
                            (if (seq map__27835) (first map__27835) {}))
                          map__27835)
             path (get map__27835 :path)
             eviction_threshold_mb (get map__27835 :eviction-threshold-mb)
             eviction_file_window (get map__27835 :eviction-file-window 10000)
             concurrency (get map__27835 :concurrency 8)
             port (get map__27835 :port 11211)
             host (get map__27835 :host)
             eviction_interval_secs (get map__27835 :eviction-interval-secs 60)
             ip_validator (get map__27835 :ip-validator (constantly true))
             sasl (get map__27835 :sasl)]
         (let [logger (org.slf4j.LoggerFactory/getLogger "datomic.valcache")]
           (when (.isInfoEnabled ^org.slf4j.Logger logger)
             (.info
               ^org.slf4j.Logger logger
               ...
               internal_shutdown (fn internal_shutdown
                                   ([]
                                     ...
                                     (loop [seq_27837 (seq
                                            ...
                                            count_27839 0
                                            i_27840 0]
                                       (if (< i_27840 count_27839)
                                         (let [sc (.nth
                                                    ^clojure.lang.Indexed chunk_27838
                                                    (int i_27840))]
                                           (.close
                                             ^java.nio.channels.spi.AbstractInterruptibleChannel sc)
                                           (recur seq_27837 chunk_27838 count_27839 (inc i_27840)))
                                         (let [temp__5825__auto__ (seq seq_27837)]
                                           (when temp__5825__auto__
                                             (let [seq_27837 temp__5825__auto__]
                                               (if (chunked-seq? seq_27837)
                                                 (let [c__6090__auto__ (chunk-first seq_27837)]
                                                   (recur
                                                     (chunk-rest seq_27837)
                                                     c__6090__auto__
                                                     (int (count c__6090__auto__))
                                                     (int 0)))
                                                 (let [sc (first seq_27837)]
                                                   (.close
                                                     ^java.nio.channels.spi.AbstractInterruptibleChannel sc)
                                                   (recur (next seq_27837) nil 0 0))))))))))
                                 ...
                                 (try
                                   (do
                                     ...
                                     (when (if sasl (sasl-loop sasl sc sem) true)
                                       (loop [hb (ByteBuffer/wrap (byte-array 24))]
                                         (let [map__27845 (read-header sc hb)
                                               map__27845 (if (seq? map__27845)
                                                            (if
                                                              (next map__27845)
                                                              (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                                                                (to-array map__27845))
   ...
   (fn direct_init
     ([p__27857]
       (let [map__27858 p__27857
             map__27858 (if (seq? map__27858)
                          (if (next map__27858)
                            (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                              (to-array map__27858))
                            (if (seq map__27858) (first map__27858) {}))
                          map__27858)
             path (get map__27858 :path)
             eviction_threshold_mb (get map__27858 :eviction-threshold-mb)
             concurrency (get map__27858 :concurrency 8)
             eviction_interval_secs (get map__27858 :eviction-interval-secs 60)
             eviction_file_window (get map__27858 :eviction-file-window 10000)]
             ...
             (fn fn__27859
               ([]
                 (eviction-loop
                   ...
                   shutdown_requested)))
             "valcache-eviction-loop")
           (fn fn__27861 ([] (reset! shutdown_requested true))))))))
  (reset-meta!
    #'direct-init
    (assoc
      {:arglists
  ...
  (def direct-put
   (fn direct_put
     ([root k v]
       ...
       (let [temp__5825__auto__ (full-path root k)]
         (when temp__5825__auto__
           (let [path temp__5825__auto__ tmp_path (full-path root (str (UUID/randomUUID)))]
             (try
               (do
                 (with-open [fc (FileChannel/open
                                  ^java.nio.file.Path tmp_path
