...
(do
  ...
  (defn seek-seg-id
    ([log t]
      (let [vec__17098 (seek-seg-path log t)
            dirid (nth vec__17098 (int 0) nil)
            segid (nth vec__17098 (int 1) nil)]
        segid)))
  (reset-meta!
    #'seek-seg-id
  ...
  (defn fressianed-txes-length
    ([fressianed_txes]
      (apply
        +
        (map
          (fn fn__17259 ([tx] (java.lang.Integer/valueOf (int (.length (:fressianed-tx tx))))))
          fressianed_txes))))
  (reset-meta!
    ...
    (assoc
      ...
      :ns
      *ns*))
  (extend java.util.Map LogKey {:log-key (fn fn__17262 ([_] (.get ^java.util.Map _ :t)))})
  (extend java.lang.Long LogKey {:log-key (fn fn__17264 ([_] _))})
  ...
  (defn tail-pod-key
    ([cs]
      (let [temp__5825__auto__ (cluster/dbId cs)]
        (when temp__5825__auto__ (let [dbid temp__5825__auto__] (str "pod-log-tail/" dbid))))))
  (reset-meta!
    #'tail-pod-key
    ...
    ([cs]
      (let [temp__5825__auto__ (cluster/dbId cs)]
        (when temp__5825__auto__
          (let [dbid temp__5825__auto__] (str "ref-log-root/" (cluster/dbId cs)))))))
  (reset-meta!
    #'legacy-root-ref-key
    (assoc
  ...
  (defn normalize-desc
    ([desc cs]
      (let [rev (long (or (:d/l desc) 1)) G__17306 rev]
        (case
          G__17306
          1
  ...
  (defn read-tail-descriptor
    ([cs]
      (let [temp__5825__auto__ (deref (cluster/get-pod cs (tail-pod-key cs)))]
        (when temp__5825__auto__
          (let [desc temp__5825__auto__
  ...
  (defn claim-log
    ([cs desc]
      (let [temp__5825__auto__ (write-tail-descriptor cs (inc-rev desc) nil)]
        (when temp__5825__auto__
          (let [new_desc temp__5825__auto__]
  ...
  (defn zip-and-create
    ([cs uuid buf write_type]
      (let [gz_buf (io/gzip-buffer buf)
            kw (let [pred__17316 = expr__17317 write_type]
                 (if (^clojure.lang.IFn pred__17316 :root expr__17317)
                   :LogTreeRootBytesWritten
                   (if (^clojure.lang.IFn pred__17316 :dir expr__17317)
                     :LogTreeDirBytesWritten
                     (if (^clojure.lang.IFn pred__17316 :leaf expr__17317)
                       :LogTreeLeafBytesWritten
                       (if (^clojure.lang.IFn pred__17316 :excise expr__17317)
  ...
  (defn tail-byte-count
    ([p__17349]
      (let [map__17350 p__17349
            map__17350 (if (seq? map__17350)
                         (if (next map__17350)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__17350))
                           (if (seq map__17350) (first map__17350) {}))
                         map__17350)
            bufs (get map__17350 :bufs)]
  ...
  (defn tail-ts
    ([p__17352]
      (let [map__17353 p__17352
            map__17353 (if (seq? map__17353)
                         (if (next map__17353)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__17353))
                           (if (seq map__17353) (first map__17353) {}))
                         map__17353)
            txes (get map__17353 :txes)]
        (map :t txes))))
  (reset-meta!
    #'tail-ts
    (assoc
  ...
  (defn tail-empty?
    ([p__17355]
      (let [map__17356 p__17355
            map__17356 (if (seq? map__17356)
                         (if (next map__17356)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__17356))
                           (if (seq map__17356) (first map__17356) {}))
                         map__17356)
            txes (get map__17356 :txes)]
        (empty? txes))))
  (reset-meta!
    #'tail-empty?
    (assoc
      ...
      *ns*))
  (defn extend-tail
    ([p__17358 new_txes new_bufs]
      (let [map__17359 p__17358
            map__17359 (if (seq? map__17359)
                         (if (next map__17359)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__17359))
                           (if (seq map__17359) (first map__17359) {}))
                         map__17359)
            txes (get map__17359 :txes)
            bufs (get map__17359 :bufs)]
  ...
  (defn since
    ([p__17361 since_t]
      (let [map__17362 p__17361
            map__17362 (if (seq? map__17362)
                         (if (next map__17362)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__17362))
                           (if (seq map__17362) (first map__17362) {}))
                         map__17362)
            tail map__17362
            txes (get map__17362 :txes)
            bufs (get map__17362 :bufs)]
        (if (or (not since_t) (tail-empty? tail) (< since_t (first (tail-ts tail))))
          tail
          (let [new_txes (loop [txes txes]
                           (let [vec__17364 txes
                                 seq__17365 (seq vec__17364)
                                 first__17366 (first seq__17365)
                                 seq__17365 (next seq__17365)
                                 tx first__17366
                                 more seq__17365]
  ...
  (deftype
    ...
    (log-seg-seq
      [this]
      (let [iter__6398__auto__ (fn iter__17391
                                 ([s__17392]
                                   (lazy-seq
                                     (loop [s__17392 s__17392]
                                       (let [temp__5825__auto__ (seq s__17392)]
                                         (when temp__5825__auto__
                                           (let [xs__6385__auto__ temp__5825__auto__
                                             ...
                                             (if fs__6395__auto__
                                               (concat
                                                 fs__6395__auto__
                                                 (^clojure.lang.IFn iter__17391 (rest s__17392)))
                                               (recur (rest s__17392))))))))))]
        (^clojure.lang.IFn iter__6398__auto__
          (range (long ridx) (java.lang.Integer/valueOf (int (count root_val)))))))
    (log-dir-seq
      [this]
      (let [iter__6398__auto__ (fn iter__17378
                                 ([s__17379]
                                   (lazy-seq
                                     (let [s__17379 s__17379 temp__5825__auto__ (seq s__17379)]
                                       (when temp__5825__auto__
                                         (let [s__17379 temp__5825__auto__]
                                           (if (chunked-seq? s__17379)
                                             (let [c__6396__auto__ (chunk-first s__17379)
                                                   size__6397__auto__ (int (count c__6396__auto__))
                                                   b__17381 (chunk-buffer
                                                              (java.lang.Integer/valueOf
                                                                (int size__6397__auto__)))]
                                               (if (loop [i__17380 (int 0)]
                                                     (if (< i__17380 size__6397__auto__)
                                                       (let [ri
                                                             (.nth
                                                               ^clojure.lang.Indexed c__6396__auto__
                                                               (int i__17380))]
                                                 ...
                                                 (chunk-cons
                                                   (chunk b__17381)
                                                   (^clojure.lang.IFn iter__17378
                                                     (chunk-rest s__17379)))
                                                 (chunk-cons (chunk b__17381) nil)))
                                             (let [ri (first s__17379)]
                                               (cons
                                                 (common/getx
                                                   lookup
                                                   (:uuid
  ...
  (defrecord
    ...
    (adopt-root
      ...
      (let [tail (since tail basis_t)
            ...
            temp__5823__auto__ (write-tail-descriptor cs proposed_desc pod_buf)]
    ...
    (append
      ...
      (let [bufs (map :fressianed-tx msgs)
            ...
            log (let [m_17439 {:event :log/add-next, :txids ids, :firstT (:t (first txes))}
                      ___8598__auto__ (let [logger (org.slf4j.LoggerFactory/getLogger
                                                     "datomic.log")]
                                        (when (.isDebugEnabled ^org.slf4j.Logger logger)
                                          (.debug
                                            ^org.slf4j.Logger logger
                                            (logger/process (assoc m_17439 :phase :begin))))
                                        nil)
                      start__8599__auto__ (java.lang.System/nanoTime)
                      result__8600__auto__ (try
                                             {:returned
                                              (let [temp__5823__auto__ (write-tail-descriptor
                                                                         cs
                                                                         (update-in
                                                                           desc
                                                                           [:rev]
                                                                         ...
                                                                         (io/unchunk bufs))]
                                                (if temp__5823__auto__
                                                  (let [new_desc temp__5823__auto__]
                      ...
                      elapsed_17440 (- (java.lang.System/nanoTime) start__8599__auto__)
                      msec_17441 (logger/format-as-msec (long elapsed_17440))]
                  (monitor/add-stat :LogWriteMsec msec_17441)
                  (let [endmsg__8602__auto__ (merge
                                               (assoc m_17439 :msec msec_17441 :phase :end)
    ...
    (val-keys
      [this]
      (tree-seq
        (fn fn__17433 ([k] (instance? datomic.log.LogDir (first (get olookup k)))))
        (fn fn__17435
          ([k] (map (fn fn__17436 ([ld] (cluster/uuid->val-key (:uuid ld)))) (get olookup k))))
        (cluster/uuid->val-key (get-root-id this))))
    (get-root-val [this] (vec (common/getx olookup (get-root-id this))))
    (get-root-id [this] (:d/r desc))
    (claim
      [this cs]
      (let [temp__5825__auto__ (claim-log cs desc)]
        (when temp__5825__auto__ (let [new_desc temp__5825__auto__] (assoc this :desc new_desc)))))
    (seek-seg-path
      [this t]
  ...
  (defn write-new-log
    ([cs]
      (let [vec__17468 (repeatedly (fn fn__17471 ([] (common/rand-uuid))))
            new_root_id (nth vec__17468 (int 0) nil)
            new_tail_id (nth vec__17468 (int 1) nil)
            ...
            temp__5825__auto__ (write-tail-descriptor cs proposed_desc BEGIN_OPEN_LIST)]
        (when temp__5825__auto__ (let [desc temp__5825__auto__] [desc tail])))))
  (reset-meta!
    #'write-new-log
    (assoc {:arglists (clojure.core/list ['cs]), :column (int 1)} :name 'write-new-log :ns *ns*))
  (defn create-new-log
    ([cs olookup]
      (let [temp__5825__auto__ (write-new-log cs)]
        (when temp__5825__auto__
          (let [vec__17475 temp__5825__auto__
                desc (nth vec__17475 (int 0) nil)
                tail (nth vec__17475 (int 1) nil)]
  ...
  (defn find-log
    ([cs olookup]
      (let [temp__5825__auto__ (read-tail-descriptor cs)]
        (when temp__5825__auto__
          (let [vec__17481 temp__5825__auto__
                desc (nth vec__17481 (int 0) nil)
                buf (nth vec__17481 (int 1) nil)
                tail (load-tail buf)]
            (create-log-impl olookup desc tail))))))
  (reset-meta!
    #'find-log
  ...
  (defn catchup-tx
    ([p__17487 tx]
      (let [map__17488 p__17487
            map__17488 (if (seq? map__17488)
                         (if (next map__17488)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__17488))
                           (if (seq map__17488) (first map__17488) {}))
                         map__17488)
            db (get map__17488 :db)
            size (get map__17488 :size)
  ...
  (defn catchup
    ([db log catchup_ft]
      (let [start (java.lang.System/currentTimeMillis)
            ...
            map__17491 (reduce catchup-tx {:db db, :size 0} log_txes)
            map__17491 (if (seq? map__17491)
                         (if (next map__17491)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__17491))
                           (if (seq map__17491) (first map__17491) {}))
                         map__17491)
            result map__17491
            db (get map__17491 :db)
            size (get map__17491 :size)
            elapsed (- (java.lang.System/currentTimeMillis) start)
            db (if catchup_ft
                 (let [m_17494 {:event :log/catchup-fulltext}
                       ___8598__auto__ (let [logger (org.slf4j.LoggerFactory/getLogger
                                                      "datomic.log")]
                                         (when (.isInfoEnabled ^org.slf4j.Logger logger)
                                           (.info
                                             ^org.slf4j.Logger logger
                                             (logger/process (assoc m_17494 :phase :begin))))
                                         nil)
                       start__8599__auto__ (java.lang.System/nanoTime)
                       result__8600__auto__ (try
                                              {:returned
                                              ...
                                              (catch
                                                ...
                                                t__8601__auto__
                                                {:threw t__8601__auto__}))
                       elapsed_17495 (- (java.lang.System/nanoTime) start__8599__auto__)
                       msec_17496 (logger/format-as-msec (long elapsed_17495))]
                   (let [endmsg__8602__auto__ (merge
                                                (assoc m_17494 :msec msec_17496 :phase :end)
                                                (when (:threw result__8600__auto__)
                                                  {:threw (class (:threw result__8600__auto__))}))
                         logger (org.slf4j.LoggerFactory/getLogger "datomic.log")]
                     (when (.isInfoEnabled ^org.slf4j.Logger logger)
  ...
  (defn ensure-index-and-log
    ([cluster olookup db_id]
      (let [m_17504 {:event :transactor/ensure-index-and-log, :db-id db_id}
            ___8598__auto__ (let [logger (org.slf4j.LoggerFactory/getLogger "datomic.log")]
                              (when (.isDebugEnabled ^org.slf4j.Logger logger)
                                (.debug
                                  ^org.slf4j.Logger logger
                                  (logger/process (assoc m_17504 :phase :begin))))
                              nil)
            start__8599__auto__ (java.lang.System/nanoTime)
            result__8600__auto__ (try
                                   {:returned
                                   ...
                                   (catch
                                     ...
                                     t__8601__auto__
                                     {:threw t__8601__auto__}))
            elapsed_17505 (- (java.lang.System/nanoTime) start__8599__auto__)
            msec_17506 (logger/format-as-msec (long elapsed_17505))]
        (let [endmsg__8602__auto__ (merge
                                     (assoc m_17504 :msec msec_17506 :phase :end)
                                     (when (:threw result__8600__auto__)
                                       {:threw (class (:threw result__8600__auto__))}))
              logger (org.slf4j.LoggerFactory/getLogger "datomic.log")]
          (when (.isDebugEnabled ^org.slf4j.Logger logger)
        ...
        (fn fn__17518
          ([s p]
            (into
              s
              (map
                (fn fn__17519 ([p1__17517#] (long (.getT ^datomic.impl.db.IDatum p1__17517#))))
                (x/datoms p)))))
        #{}
        xpreds)))
  (reset-meta!
  ...
  (defn excise-dir-map
    ([log ts]
      (reduce
        (fn fn__17525
          ([m p__17524]
            (let [vec__17526 p__17524
                  dirid (nth vec__17526 (int 0) nil)
                  segid (nth vec__17526 (int 1) nil)]
              (if (contains? (get m dirid) segid)
                m
                (assoc m dirid (conj (get m dirid #{}) segid))))))
        {}
        (map (fn fn__17530 ([p1__17523#] (seek-seg-path log p1__17523#))) ts))))
  (reset-meta!
    #'excise-dir-map
    (assoc
  ...
  (defn write-excised-log
    ([cs lookup xpreds ts dir_map]
      (reduce
        (fn fn__17535
          ([m p__17534]
            (let [vec__17536 p__17534
                  dirid (nth vec__17536 (int 0) nil)
                  segids (nth vec__17536 (int 1) nil)
                  excise? (fn excise_QMARK_
                            ([d]
                              (some
                                (fn fn__17543 ([p1__17533#] (x/remove? p1__17533# d)))
                                xpreds)))
                  dir (common/getx lookup dirid)
                  vec__17539 (reduce
                               (fn fn__17547
                                 ([p__17546 direntry]
                                   (let [vec__17548 p__17546
                                         m (nth vec__17548 (int 0) nil)
                                         newdir (nth vec__17548 (int 1) nil)
                               ...
                               [m []]
                               dir)
                  m (nth vec__17539 (int 0) nil)
                  newdir (nth vec__17539 (int 1) nil)
      ...
      (let [ts (excise-ts log xpreds)
            ...
            cs (if pario
                 (cluster/queueing-writer
                   ...
                   (fn fn__17559 ([p1__17558#] (monitor/add-stat :ExciseIOQueueCount p1__17558#))))
                 cs)
  ...
  (defn excise-root
    ([cs lookup current_root_id p__17564]
      (let [map__17565 p__17564
            map__17565 (if (seq? map__17565)
                         (if (next map__17565)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__17565))
                           (if (seq map__17565) (first map__17565) {}))
                         map__17565)
            dir_map (get map__17565 :dir-map)
            replacements (get map__17565 :replacements)
            log (log-tree lookup current_root_id)
            root (get-root-val log)
            dirs (into
                   #{}
                   (map
                     (fn fn__17566 ([p1__17562#] (.-uuid ^datomic.log.LogDir p1__17562#)))
                     root))
                            ...
                            (fn fn__17569
                              ([newdir direntry]
                                (let [eid (.-uuid ^datomic.log.LogDir direntry)]
                                  (conj
                                    newdir
                                    (let [temp__5823__auto__ (^clojure.lang.IFn replacements eid)]
                                      (if temp__5823__auto__
                                        (let [neweid temp__5823__auto__]
                                          (datomic.log.LogDir.
                                            (long (.-t ^datomic.log.LogDir direntry))
                                            neweid))
                                        direntry))))))
            ...
            cs (if pario
                 (cluster/queueing-writer
                   ...
                   pario
                   cluster/BOUNDING_TIMEOUT_MSEC
                   (fn fn__17573 ([p1__17563#] (monitor/add-stat :ExciseIOQueueCount p1__17563#))))
                 cs)
                                                          ...
                                                          (first
                                                            (common/getx
                                                              ...
                                                              (^clojure.lang.IFn replacements
                                                                (first segids)))))))))
                                     dirid (nth vec__17576 (int 0) nil)
                                     _ (nth vec__17576 (int 1) nil)]
          ...
          (^java.util.Iterator iterator
            ...
            (iter/iterator
              (let [ret (seek-tx log start)]
                (if end
                  (iter/take-while (fn fn__17606 ([p1__17604#] (< (:t p1__17604#) end))) ret)
                  ret))))))))
  (reset-meta!
    #'tx-range
    (assoc
  ...
  (defn create-log-val
    ([cs olookup db]
      ...
      (let [temp__5825__auto__ (deref (cluster/get-pod-meta cs (tail-pod-key cs)))]
        (when temp__5825__auto__
          (let [desc temp__5825__auto__
                desc (normalize-desc desc cs)
  ...
  (defn max-t
    ([datoms]
      (reduce (fn fn__17644 ([n datom] (max n (db/eid->eidx (long (:tx datom)))))) 0 datoms)))
  (reset-meta!
    #'max-t
  ...
  (defn last-tree-tx
    ...
    ([p__17650]
      (let [map__17651 p__17650
            map__17651 (if (seq? map__17651)
                         (if (next map__17651)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__17651))
                           (if (seq map__17651) (first map__17651) {}))
                         map__17651)
            log map__17651
            olookup (get map__17651 :olookup)]
        ...
        ([rf]
          (let [weight_ref (volatile! 0) part_ref (volatile! [])]
            (fn fn__17656
              ([] (^clojure.lang.IFn rf))
              ([result]
                (let [temp__5823__auto__ (seq (deref part_ref))]
                  (if temp__5823__auto__
                    (let [part temp__5823__auto__]
        ...
        ([rf]
          (let [tail_ref (volatile! nil)]
            (fn fn__17662
              ([] (^clojure.lang.IFn rf))
              ([result]
                (let [vec__17663 (deref tail_ref)
                      p1 (nth vec__17663 (int 0) nil)
                      p2 (nth vec__17663 (int 1) nil)]
                  (if p1
              ...
              ([result input]
                (let [vec__17667 (deref tail_ref)
                      p1 (nth vec__17667 (int 0) nil)
                      p2 (nth vec__17667 (int 1) nil)]
      ...
      (let [weigh (comp io/remaining second)]
        (sequence
          (comp
            ...
            (combine-last-if
              (fn fn__17673 ([ftxes] (< (apply + (map weigh ftxes)) (quot target_size 2))))
              into)
            (map
              (fn fn__17675
                ([ftxes]
    ...
    ([olookup root]
      (when (> (count root) (long (deref coalesce-threshold*)))
        (let [sample_indexes (take
                               ...
                               (distinct
                                 (repeatedly
                                   (fn fn__17687 ([] (rand-int (long (- (count root) 3))))))))
              sample (fn sample
                       ([i]
                         (let [vec__17690 [(:uuid (nth root (int ^java.lang.Number i)))
                                           (:uuid (nth root (int (inc i))))]
                               u0 (nth vec__17690 (int 0) nil)
                               u1 (nth vec__17690 (int 1) nil)
                               d0 (vec (cache/getx-uncached olookup u0))
                               d1 (vec (cache/getx-uncached olookup u1))
                               t (+ (count d0) (count d1))]
                           {:i i, :total (long t), :u0 u0, :u1 u1, :d0 d0, :d1 d1})))
              vec__17683 (map sample sample_indexes)
              s0 (nth vec__17683 (int 0) nil)
              s1 (nth vec__17683 (int 1) nil)
              map__17686 (min-key :total s0 s1)
              map__17686 (if (seq? map__17686)
                           (if (next map__17686)
                             (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                               (to-array map__17686))
                             (if (seq map__17686) (first map__17686) {}))
                           map__17686)
              data map__17686
              total (get map__17686 :total)]
  ...
  (defn coalesce
    ([cs root p__17695]
      (let [map__17696 p__17695
            map__17696 (if (seq? map__17696)
                         (if (next map__17696)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__17696))
                           (if (seq map__17696) (first map__17696) {}))
                         map__17696)
            u0 (get map__17696 :u0)
            u1 (get map__17696 :u1)
            d0 (get map__17696 :d0)
            d1 (get map__17696 :d1)
            i (get map__17696 :i)
  ...
  (defn coalesce-dirs
    ([cs olookup root]
      (let [start (java.lang.System/nanoTime)
            rv (let [temp__5823__auto__ (select-dirs-to-merge olookup root)]
                 (if temp__5823__auto__
                   (let [data temp__5823__auto__] (coalesce cs root data))
                   [root [] []]))
    ...
    ([cs olookup root_id target_dir_count leaf_segs]
      (let [leaf_ts (map first leaf_segs)
            ...
            leaf_ids (repeatedly
                       ...
                       common/rand-uuid)
            vec__17701 (repeatedly common/rand-uuid)
            new_root_id (nth vec__17701 (int 0) nil)
            new_dir_id (nth vec__17701 (int 1) nil)
            ...
            vec__17704 (coalesce-dirs cs olookup new_root)
            new_root (nth vec__17704 (int 0) nil)
            coalesce_in_flight (nth vec__17704 (int 1) nil)
            coalesce_garbage (nth vec__17704 (int 2) nil)
            garbage_ids (cond->
        ...
        (if (every? (fn fn__17710 ([p1__17700#] (= p1__17700# :created))) (map deref results))
  ...
  (defn segment
    ([cluster olookup]
      (let [map__17713 (find-log cluster olookup)
            map__17713 (if (seq? map__17713)
                         (if (next map__17713)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__17713))
                           (if (seq map__17713) (first map__17713) {}))
                         map__17713)
            log map__17713
            desc (get map__17713 :desc)
            tail (get map__17713 :tail)]
        (if (tail-empty? tail)
          log
          (let [map__17714 (extend-tree
                             cluster
                             olookup
                             (:d/r desc)
                             dir-threshold
                             (create-leaves segment-threshold tail))
                map__17714 (if (seq? map__17714)
                             (if (next map__17714)
                               (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                                 (to-array map__17714))
                               (if (seq map__17714) (first map__17714) {}))
                             map__17714)
                root_id (get map__17714 :root-id)]
