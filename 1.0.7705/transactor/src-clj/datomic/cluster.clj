(do
  ...
  (.bindRoot
    ...
    (fn path
      ([args]
        (let [map__9122 args
              map__9122 (if (seq? map__9122)
                          (if (next map__9122)
                            (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                              (to-array map__9122))
                            (if (seq map__9122) (first map__9122) {}))
                          map__9122)
              tenant (get map__9122 :tenant)
              db (get map__9122 :db)
              partition (get map__9122 :partition)
              key (get map__9122 :key)]
  ...
  (defn reset-ref
    ([cluster k v]
      (let [map__9323 (deref (get-ref cluster k))
            map__9323 (if (seq? map__9323)
                        (if (next map__9323)
                          (clojure.lang.PersistentArrayMap/createAsIfByAssoc (to-array map__9323))
                          (if (seq map__9323) (first map__9323) {}))
                        map__9323)
            prev map__9323
            rev (get map__9323 :rev)
            m_9324 {:event :kv-cluster/reset-ref, :key k, :from (:key prev), :to v}
            ___8598__auto__ (let [logger (org.slf4j.LoggerFactory/getLogger "datomic.cluster")]
                              (when (.isInfoEnabled ^org.slf4j.Logger logger)
                                (.info
                                  ^org.slf4j.Logger logger
                                  (logger/process (assoc m_9324 :phase :begin))))
                              nil)
            ...
            elapsed_9325 (- (java.lang.System/nanoTime) start__8599__auto__)
            msec_9326 (logger/format-as-msec (long elapsed_9325))]
        (let [endmsg__8602__auto__ (merge
                                     (assoc m_9324 :msec msec_9326 :phase :end)
  ...
  (defn touch-ref
    ([cluster k]
      (let [map__9334 (deref (get-ref cluster k))
            map__9334 (if (seq? map__9334)
                        (if (next map__9334)
                          (clojure.lang.PersistentArrayMap/createAsIfByAssoc (to-array map__9334))
                          (if (seq map__9334) (first map__9334) {}))
                        map__9334)
            prev map__9334
            rev (get map__9334 :rev)
            m_9335 {:event :kv-cluster/touch-ref, :from prev}
            ___8598__auto__ (let [logger (org.slf4j.LoggerFactory/getLogger "datomic.cluster")]
                              (when (.isWarnEnabled ^org.slf4j.Logger logger)
                                (.warn
                                  ^org.slf4j.Logger logger
                                  (logger/process (assoc m_9335 :phase :begin))))
                              nil)
            start__8599__auto__ (java.lang.System/nanoTime)
            result__8600__auto__ (try
                                   {:returned
                                   ...
                                   (catch
                                     ...
                                     t__8601__auto__
                                     {:threw t__8601__auto__}))
            elapsed_9336 (- (java.lang.System/nanoTime) start__8599__auto__)
            msec_9337 (logger/format-as-msec (long elapsed_9336))]
        (let [endmsg__8602__auto__ (merge
                                     (assoc m_9335 :msec msec_9337 :phase :end)
                                     (when (:threw result__8600__auto__)
                                       {:threw (class (:threw result__8600__auto__))}))
              logger (org.slf4j.LoggerFactory/getLogger "datomic.cluster")]
          (when (.isWarnEnabled ^org.slf4j.Logger logger)
            (.warn ^org.slf4j.Logger logger (logger/process endmsg__8602__auto__)))
          nil)
        (if (contains? result__8600__auto__ :returned)
          (:returned result__8600__auto__)
          (do (throw (:threw result__8600__auto__)) nil)))))
  ...
  (defn reset-pod
    ([cluster k buf metamap]
      (let [map__9345 (deref (get-pod cluster k))
            map__9345 (if (seq? map__9345)
                        (if (next map__9345)
                          (clojure.lang.PersistentArrayMap/createAsIfByAssoc (to-array map__9345))
                          (if (seq map__9345) (first map__9345) {}))
                        map__9345)
            rev (get map__9345 :rev)]
  ...
  (defn write-vals*
    ([cs source vmap]
      (let [event (let [G__9359 source]
                    (case G__9359 :index :index/write-val :clusterfs :clusterfs/write-val))
            metric (let [G__9360 source]
                     (case G__9360 :index :IndexWriteNsec :clusterfs :FulltextWriteNsec))
            pacing (deref segment-pacing-msec)
            rets (mapv
                   (fn fn__9362
                     ([p__9361]
                       (let [vec__9363 p__9361
                             k (nth vec__9363 (int 0) nil)
                             v (nth vec__9363 (int 1) nil)]
        ...
        (dorun
          (map
            (fn fn__9368
              ([p__9367]
                (let [vec__9369 p__9367
                      start (nth vec__9369 (int 0) nil)
                      ret (nth vec__9369 (int 1) nil)]
  ...
  (defn write-vals
    ([cs source vmap]
      (let [m_9374 {:event :cluster/write-vals,
                    :vcnt (java.lang.Integer/valueOf (int (count vmap)))}
            ___8598__auto__ (let [logger (org.slf4j.LoggerFactory/getLogger "datomic.cluster")]
                              (when (.isDebugEnabled ^org.slf4j.Logger logger)
                                (.debug
                                  ^org.slf4j.Logger logger
                                  (logger/process (assoc m_9374 :phase :begin))))
                              nil)
            start__8599__auto__ (java.lang.System/nanoTime)
            result__8600__auto__ (try
                                   {:returned
                                    (write-vals*
                                      ...
                                      (map
                                        (fn fn__9379
                                          ([p__9378]
                                            (let [vec__9380 p__9378
                                                  k (nth vec__9380 (int 0) nil)
                                                  v (nth vec__9380 (int 1) nil)]
                                              [(uuid->val-key k) v])))
                                        vmap))}
                                   (catch
                                     java.lang.Throwable
                                     t__8601__auto__
                                     {:threw t__8601__auto__}))
            elapsed_9375 (- (java.lang.System/nanoTime) start__8599__auto__)
            msec_9376 (logger/format-as-msec (long elapsed_9375))]
        (let [endmsg__8602__auto__ (merge
                                     (assoc m_9374 :msec msec_9376 :phase :end)
                                     (when (:threw result__8600__auto__)
                                       {:threw (class (:threw result__8600__auto__))}))
              logger (org.slf4j.LoggerFactory/getLogger "datomic.cluster")]
          (when (.isDebugEnabled ^org.slf4j.Logger logger)
