(do
  ...
  (extend
    ...
    {:qualified-name
     (fn fn__30669
       ([kw]
         (let [temp__5823__auto__ (namespace kw)]
           (if temp__5823__auto__
             (let [ns temp__5823__auto__] (str ns "/" (name kw)))
             (name kw)))))})
  ...
  (extend
    ...
    {:to-metric-data-helper
     (fn fn__30690
       ([v k]
         (let [map__30691 v
               map__30691 (if (seq? map__30691)
                            (if (next map__30691)
                              (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                                (to-array map__30691))
                              (if (seq map__30691) (first map__30691) {}))
                            map__30691)
               lo (get map__30691 :lo)
               hi (get map__30691 :hi)
               sum (get map__30691 :sum)
               count (get map__30691 :count)]
  ...
  (defn to-metric-data
    ([statistics]
      (mapv
        (fn fn__30696
          ([p__30695]
            (let [vec__30697 p__30695
                  k (nth vec__30697 (int 0) nil)
                  v (nth vec__30697 (int 1) nil)]
  ...
  (defn create-request-map
    ([dimensions metrics]
      {:Namespace "Datomic",
       :MetricData
       (mapv
         (fn fn__30703 ([p1__30702#] (assoc p1__30702# :Dimensions dimensions)))
  ...
  (defn report-metrics
    ([client dimensions metrics]
      (let [mparts (partitioned-metrics-requests dimensions metrics)]
        (loop [seq_30707 (seq mparts) chunk_30708 nil count_30709 0 i_30710 0]
          (if (< i_30710 count_30709)
            (let [mpart (.nth ^clojure.lang.Indexed chunk_30708 (int i_30710))]
              (aws/invoke client {:op :PutMetricData, :req mpart})
              (recur seq_30707 chunk_30708 count_30709 (inc i_30710)))
            (let [temp__5825__auto__ (seq seq_30707)]
              (when temp__5825__auto__
                (let [seq_30707 temp__5825__auto__]
                  (if (chunked-seq? seq_30707)
                    (let [c__6090__auto__ (chunk-first seq_30707)]
                      (recur
                        (chunk-rest seq_30707)
                        c__6090__auto__
                        (int (count c__6090__auto__))
                        (int 0)))
                    (let [mpart (first seq_30707)]
                      (aws/invoke client {:op :PutMetricData, :req mpart})
                      (recur (next seq_30707) nil 0 0)))))))))))
  (reset-meta!
    #'report-metrics
    (assoc
  ...
  (.bindRoot
    ...
    (delay
      (create-cloudwatch-reporter
        ...
        :creds
        (let [temp__5825__auto__ (config/property "datomic.cloudwatchAccessKeyId")]
          (when temp__5825__auto__
            (let [key_id temp__5825__auto__]
