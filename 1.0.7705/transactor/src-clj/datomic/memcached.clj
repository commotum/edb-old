(do
  ...
  (def factory*
   (fn factory_STAR_
     ([p__21590]
       (let [map__21591 p__21590
             map__21591 (if (seq? map__21591)
                          (if (next map__21591)
                            (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                              (to-array map__21591))
                            (if (seq map__21591) (first map__21591) {}))
                          map__21591)
             timeout_msec (get map__21591 :timeout-msec 10)
             config_timeout_msec (get map__21591 :config-timeout-msec 100)
             username (get map__21591 :username)
             password (get map__21591 :password)
             auto_discovery (get map__21591 :auto-discovery)
  ...
  (def create-client
   (fn create_client
     ([p__21594]
       (let [map__21595 p__21594
             map__21595 (if (seq? map__21595)
                          (if (next map__21595)
                            (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                              (to-array map__21595))
                            (if (seq map__21595) (first map__21595) {}))
                          map__21595)
             args map__21595
             servers (get map__21595 :servers)]
         (let [logger (org.slf4j.LoggerFactory/getLogger "datomic.memcached")]
  ...
  (deftype
    ...
    (rc-set
      [this k ttl v]
      (let [G__21665 (.set (deref client_ref) ^java.lang.String k (int (long ttl)) v)]
        (.addListener
          ^datomic.spy.memcached.internal.OperationFuture G__21665
          (op-listener
            (fn fn__21666
              ([p1__21661#]
                (try
  ...
  (def create-cache
   (fn create_cache
     ([p__21673]
       (let [map__21674 p__21673
             map__21674 (if (seq? map__21674)
                          (if (next map__21674)
                            (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                              (to-array map__21674))
                            (if (seq map__21674) (first map__21674) {}))
                          map__21674)
             args map__21674
             shutdown_client? (get map__21674 :shutdown-client? true)
             record_kv (get map__21674 :record-kv monitor/add-stat)
             client (get map__21674 :client)
             metric_names (get map__21674 :metric-names memcached-metric-names)
             vec__21675 metric_names
             io_counter (nth vec__21675 (int 0) nil)
             io_latency (nth vec__21675 (int 1) nil)
             hit_counter (nth vec__21675 (int 2) nil)
             get_succeeded (nth vec__21675 (int 3) nil)
             get_failed (nth vec__21675 (int 4) nil)
             get_missed (nth vec__21675 (int 5) nil)
             get_timeout (nth vec__21675 (int 6) nil)
             get_queue_full (nth vec__21675 (int 7) nil)
             put_succeeded (nth vec__21675 (int 8) nil)
             put_failed (nth vec__21675 (int 9) nil)
             ...
             (if (fits-in-memcached? v)
               (try
                 (let [G__21691 (rc-set
                   ...
                   (.addListener
                     ^datomic.spy.memcached.internal.OperationFuture G__21691
                     (op-listener
           ...
           (valAt
             ...
             (do
               ...
               (let [record_latencies (fn record_latencies
                                        ([nanos p__21682]
                                          (let [vec__21684 p__21682
                                                v (nth vec__21684 (int 0) nil)
                                                ex (nth vec__21684 (int 1) nil)]
                                            (when-not v
                                            ...
                                            (let [temp__5825__auto__ (cond
                                              ...
                                              (when temp__5825__auto__
                                                (let [k temp__5825__auto__]
                                                  (^clojure.lang.IFn record_kv k nanos)))))))
                     vec__21679 (let [start__8781__auto__ (java.lang.System/nanoTime)
                     ...
                     result (nth vec__21679 (int 0) nil)
                     ex (nth vec__21679 (int 1) nil)]
  ...
  (defn start-local-memcached-from-config
    ([]
      (let [temp__5825__auto__ (config/local-memcached-args)]
        (when temp__5825__auto__
          (let [local_memcached_args temp__5825__auto__]
            (if (config/folsom?)
              ...
              (create-cache
                {:client
                 (create-recovering-client
                   (fn fn__21696 ([] (create-client local_memcached_args)))),
  ...
  (defn start-memcached-from-config
    ([]
      (let [temp__5825__auto__ (config/memcached-args)]
        (when temp__5825__auto__
          (let [memcached_args temp__5825__auto__]
            (if (config/folsom?)
              (require/require-and-run 'datomic.memcached.prod-266/create-cache memcached_args)
              (create-cache
                {:client
                 (create-recovering-client (fn fn__21700 ([] (create-client memcached_args)))),
                 :metric-names memcached-metric-names})))))))
  (reset-meta!
