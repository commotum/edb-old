  ...
  (defn log-completion!
    ([ids]
      (let [written_at (java.lang.System/nanoTime)]
        (loop [seq_17029 (seq ids) chunk_17030 nil count_17031 0 i_17032 0]
          (if (< i_17032 count_17031)
            (let [id (.nth ^clojure.lang.Indexed chunk_17030 (int i_17032))]
              (let [temp__5825__auto__ (cache/remove log-event-map id)]
                (when temp__5825__auto__
                  (let [map__17033 temp__5825__auto__
                        map__17033 (if (seq? map__17033)
                                     (if (next map__17033)
                                       (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                                         (to-array map__17033))
                                       (if (seq map__17033) (first map__17033) {}))
                                     map__17033)
                        tx_info map__17033
                        read_at (get map__17033 :read-at)
                        started_at (get map__17033 :started-at)
                        applied_at (get map__17033 :applied-at)
                        tx_nsec (let [and__5600__auto__ written_at]
                                  (if (long and__5600__auto__)
                                    (and read_at (- written_at read_at))
                                    (long and__5600__auto__)))
                        msec (let [and__5600__auto__ written_at]
                               (if (long and__5600__auto__)
                        ...
                        (.info
                          ...
                          (logger/process
                            (select-keys
                              ...
                              loggable-keys))))
                      nil))))
              (recur seq_17029 chunk_17030 count_17031 (inc i_17032)))
            (let [temp__5825__auto__ (seq seq_17029)]
              (when temp__5825__auto__
                (let [seq_17029 temp__5825__auto__]
                  (if (chunked-seq? seq_17029)
                    (let [c__6090__auto__ (chunk-first seq_17029)]
                      (recur
                        (chunk-rest seq_17029)
                        c__6090__auto__
                        (int (count c__6090__auto__))
                        (int 0)))
                    (let [id (first seq_17029)]
                      (let [temp__5825__auto__ (cache/remove log-event-map id)]
                        (when temp__5825__auto__
                          (let [map__17035 temp__5825__auto__
                                map__17035 (if (seq? map__17035)
                                             (if (next map__17035)
                                               (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                                                 (to-array map__17035))
                                               (if (seq map__17035) (first map__17035) {}))
                                             map__17035)
                                tx_info map__17035
                                read_at (get map__17035 :read-at)
                                started_at (get map__17035 :started-at)
                                applied_at (get map__17035 :applied-at)
                                tx_nsec (let [and__5600__auto__ written_at]
                                          (if (long and__5600__auto__)
                                            (and read_at (- written_at read_at))
                                            (long and__5600__auto__)))
