(do
  ...
  (defn deser-key
    ([p__20143]
      (let [map__20144 p__20143
            map__20144 (if (seq? map__20144)
                         (if (next map__20144)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__20144))
                           (if (seq map__20144) (first map__20144) {}))
                         map__20144)
            key (get map__20144 :key)
            alg (get map__20144 :alg)
  ...
  (defn calc-hmac
    ([bbuf alg k]
      (let [mac (let [G__20149 (Mac/getInstance ^java.lang.String alg)]
                  (.init ^javax.crypto.Mac G__20149 ^java.security.Key k)
                  G__20149)
    ...
    ([input k op extra]
      (let [ciph (cipher (str (.getAlgorithm ^java.security.Key k) "/CBC/PKCS5Padding"))
        ...
        (.init
          ...
          (int
            (let [G__20154 op]
              (case G__20154 :encrypt Cipher/ENCRYPT_MODE :decrypt Cipher/DECRYPT_MODE)))
          ^java.security.Key k
  ...
  (defn keystore
    ([f password type]
      (with-open [is (jio/input-stream f)]
        (let [G__20158 (KeyStore/getInstance ^java.lang.String type)]
          (.load ^java.security.KeyStore G__20158 ^java.io.InputStream is ^chars password)
          G__20158)))
