...
shareable exact snapshot references, storage-independent cache-resident reads,
an opt-in SSD cache, complete I/O instrumentation and advisory transaction hints,
richer persisted-program queries/authoring and plain-data/log query sources,
integrated peer-local fulltext, partition assignment, and operating acceptance.
...
goal-archive/G1, G2 and G3 are historical
evidence, not active instructions. Keep one child active; no recursive hierarchy
or corrective parent. A scaffold or completed child is not the parent's finish
