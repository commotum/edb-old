(do
  ...
  (extend
    ...
    impl/FastCount
    {:fast-count
     (fn fn__575 ([coll] (long (.estimatedSize ^com.github.benmanes.caffeine.cache.Cache coll))))})
  (extend
    com.github.benmanes.caffeine.cache.Cache
    impl/CachePut
    {:put (fn fn__577 ([c k v] (.put ^com.github.benmanes.caffeine.cache.Cache c k v) nil))})
  (extend
    com.github.benmanes.caffeine.cache.Cache
    impl/CacheRemove
    {:remove
     (fn fn__579
       ([c k]
         (let [v (.getIfPresent ^com.github.benmanes.caffeine.cache.Cache c k)]
           ...
           v))),
     :clear (fn fn__581 ([c] (.invalidateAll ^com.github.benmanes.caffeine.cache.Cache c) nil))})
  ...
  (extend
    com.github.benmanes.caffeine.cache.Cache
    CacheGet
    {:cache-get (fn fn__601 ([this k] (.getIfPresent this k)))})
  (extend
    com.github.benmanes.caffeine.cache.LoadingCache
    CacheGet
    {:cache-get (fn fn__603 ([this k] (.get this k)))})
  (deftype
    WrappedCCache
    [cache]
    ...
    (memory-size
      [this]
      (reduce
        (fn fn__606 ([n k] (+ (+ (+ n 82) (size/memory-size k)) (size/memory-size (get this k)))))
        0
