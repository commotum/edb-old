(do
  ...
  (defn credentials
    ([creds]
      (cond
        (map? creds) (let [map__17386 creds
                           map__17386 (if (seq? map__17386)
                                        (clojure.lang.PersistentHashMap/create (seq map__17386))
                                        map__17386)
                           aws_access_key_id (get map__17386 :aws-access-key-id)
                           aws_secret_key (get map__17386 :aws-secret-key)]
  ...
  (defn endpoint-for
    ([service region]
      (let [sname (name service) rname (name region) G__17389 sname]
        (case G__17389 "iam" "iam.amazonaws.com" (str sname "." rname ".amazonaws.com")))))
  (defmethod
    d/data-to-object
      ...
      (let [o (com.amazonaws.ClientConfiguration.)]
        ...
        (when (contains? m :proxyAuthenticationMethods)
          (let [v (:proxyAuthenticationMethods m)
            ...
            (.setProxyAuthenticationMethods ^com.amazonaws.ClientConfiguration o ^java.util.List k)
            nil))
        (when (contains? m :connectionTimeout)
          (let [v (:connectionTimeout m)
            ...
            (.setConnectionTimeout ^com.amazonaws.ClientConfiguration o (int ^java.lang.Number k))
            nil))
