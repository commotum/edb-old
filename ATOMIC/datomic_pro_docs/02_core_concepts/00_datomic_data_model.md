...
site: "Datomic"
source: "https://docs.datomic.com/data-model.html"
word_count: 1699
---

A Datomic database is a set of immutable atomic facts called *datoms*. It has no tables; instead it has a universal schema of user-defined attributes, in which any entity can possess any attribute.

Datomic’s Datalog queries automatically use multiple indexes to support a variety of access patterns as well as an indelible time model, identity and uniqueness, and lookup refs.
...
For example, consider storing an inventory database in Datomic. All inventory items have a unique string identifier, so you create an `:inv/id` attribute. In addition, you create other named attributes, specifying the [types](https://docs.datomic.com/schema/schema-reference.html#db-valuetype) and [cardinalities](https://docs.datomic.com/schema/schema-reference.html#db-cardinality) of each.
...
For more information see the [schema documentation](https://docs.datomic.com/schema/schema-reference.html).
...
For more information, see the [Datomic Time Model Documentation](https://docs.datomic.com/reference/log.html).

## Identity and Uniqueness

Datomic provides a number of tools for dealing with identity and uniqueness.

- When you create a datom, Datomic automatically assigns it a database-unique number called an [entity id](https://docs.datomic.com/schema/schema-reference.html#db-id).
- The built-in [:db/ident attribute](https://docs.datomic.com/schema/schema-reference.html#db-ident) can be used for programmatic names.
- The built-in [:db/unique attribute](https://docs.datomic.com/schema/schema-reference.html#db-unique) can be added at any time. The uniqueness properties of an existing attribute also can be changed at any time (so long as the uniqueness properties are valid for the data). The:db/unique attribute can be used in the following ways:
...
- Lookup refs cannot be used in the body of a query though they can be used as inputs to a [parameterized query](https://docs.datomic.com/query/query-executing.html#query-caching).
