...
pub struct NetworkLab {
    peer_namespace: Child,
}
impl NetworkLab {
    pub fn new() -> Self {
        assert_eq!(
            std::env::var("ATOMIC_ISOLATED_NETWORK").as_deref(),
            Ok("1"),
            "run via isolated namespace helper"
        );
        assert_ne!(
            std::fs::metadata("/proc/self/ns/net").unwrap().ino(),
            std::fs::metadata("/proc/1/ns/net").unwrap().ino(),
            "refusing to configure the original network namespace"
        );
        ...
        let peer_namespace = Command::new("unshare")
            .args(["--net", "sleep", "300"])
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .spawn()
        ...
        while std::fs::metadata(format!("/proc/{}/ns/net", lab.peer_namespace.id()))
            .unwrap()
            .ino()
            == own
        {
            assert!(
                ...
                "peer network namespace did not start"
            );
            std::thread::sleep(Duration::from_millis(5));
        }
        checked(Command::new("ip").args([
            "link",
            "add",
            "atomic-left",
            "type",
        ...
        checked(Command::new("ip").args([
            ...
            "atomic-right",
            "netns",
            &lab.peer_namespace.id().to_string(),
        ]));
        ...
        checked(lab.peer_command("ip").args([
            "addr",
            "add",
            "192.0.2.2/30",
            "dev",
    ...
    pub fn peer_command(&self, program: impl AsRef<std::ffi::OsStr>) -> Command {
        ...
        command
            .args([
                "--target",
                &self.peer_namespace.id().to_string(),
                "--net",
                "--",
            ])
            .arg(program);
...
impl Drop for NetworkLab {
    fn drop(&mut self) {
        let _ = self.peer_namespace.kill();
        let _ = self.peer_namespace.wait();
    }
}
...
pub fn enter(test_name: &str) -> bool {
    if std::env::var("ATOMIC_ISOLATED_NETWORK").as_deref() == Ok("1") {
        return false;
    }
    let status = Command::new("unshare")
        ...
        .env("ATOMIC_ISOLATED_NETWORK", "1")
        .status()
        .unwrap();
    assert!(status.success(), "isolated network child failed");
    true
}
