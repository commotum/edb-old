(do
    ...
    ([path]
      (let [temp__5802__auto__ (re-find #"(.*)/([^/]*)" path)]
        (if temp__5802__auto__
          (let [vec__21832 temp__5802__auto__
                _ (nth vec__21832 (int 0) nil)
                parent (nth vec__21832 (int 1) nil)
                name (nth vec__21832 (int 2) nil)]
            [parent name])
          [nil path]))))
  (reset-meta!
    #'split-path
  ...
  (defn invert-prefix-case
    ([s]
      (str/replace
        ...
        (fn fn__21841
          ([p__21840]
            (let [vec__21842 p__21840
                  _ (nth vec__21842 (int 0) nil)
                  s (nth vec__21842 (int 1) nil)]
              (invert-case s)))))))
  (reset-meta!
    #'invert-prefix-case
    (assoc
  ...
  (deftype
    ...
    (list-keys
      [this raw_prefix]
      (let [vec__21853 (split-path raw_prefix)
            parent (nth vec__21853 (int 0) nil)
            prefix (nth vec__21853 (int 1) nil)
        ...
        (when (.isDirectory ^java.io.File base)
          {:ks
           (filter
             (fn fn__21856
               ([p1__21849#]
                 (.startsWith ^java.lang.String p1__21849# ^java.lang.String raw_prefix)))
             (map
               (fn fn__21858 ([p1__21848#] (subs (.getPath ^java.io.File p1__21848#) (long len))))
               (remove
                 (fn fn__21860 ([p1__21847#] (.isDirectory ^java.io.File p1__21847#)))
                 (file-seq base))))})))
    (store
      [this k buf]
      (let [f (jio/file root k)]
