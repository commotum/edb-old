          ...
          ([m spec]
            (let [e (datomic.excise/target db spec)]
              (if (datomic.excise/e-target? e)
                (let [vec__13675 (datomic.excise/pred-and-extent db spec)
                      epred (nth vec__13675 (int 0) nil)
                      es (nth vec__13675 (int 1) nil)]
                  (reduce (fn fn__13678 ([m e] (update m e conj epred))) m es))
                m))))
        {}
        specs)))
  (reset-meta!
            ...
            (let [temp__5804__auto__ (concat
              ...
              (when temp__5804__auto__
                (let [xpreds temp__5804__auto__]
                  (some
                    (fn fn__13684 ([p1__13682#] (datomic.excise/ep-remove? p1__13682# d)))
                    xpreds)))))
          ...
          (ep-remove?
            ...
            (let [temp__5804__auto__ (get a_>xpreds (:a d))]
              (when temp__5804__auto__
                (let [xpreds temp__5804__auto__]
                  (some
                    (fn fn__13696 ([p1__13692#] (datomic.excise/ep-remove? p1__13692# d)))
                    xpreds)))))
          (ep-datoms
            [this]
            (mapcat
              (fn fn__13694 ([xpreds] (mapcat datomic.excise/ep-datoms xpreds)))
              (vals a_>xpreds)))))))
  (reset-meta!
    #'create-as-pred
    (assoc
