(do
  ...
  (deftype
    ...
    (release-pending-syncs
      [this new_db]
      (locking lck
       (let [G__21461 (.peek ^java.util.Queue q)
             map__21462 G__21461
             map__21462 (if (seq? map__21462)
                          (clojure.lang.PersistentHashMap/create (seq map__21462))
                          map__21462)
             t (get map__21462 :t)
             prom (get map__21462 :prom)]
         (loop [G__21461 G__21461]
           (let [map__21463 G__21461
                 map__21463 (if (seq? map__21463)
                              (clojure.lang.PersistentHashMap/create (seq map__21463))
                              map__21463)
                 t (get map__21463 :t)
                 prom (get map__21463 :prom)]
        ...
        (if (and db (<= t (^clojure.lang.IFn f db)))
          ...
          (locking lck
           (cond
             ...
             (and db (<= t (:basisT db))) (let [temp__5455__auto__ (let 
                                                                     [G__21459
                                                                      (db/ts-needing-index
                                                                        db
                                                                        btype
                                            ...
                                            (if temp__5455__auto__
                                              (let [actual_t temp__5455__auto__]
      ...
      (let [nextT (.nextT ^datomic.Database db)
            temp__5455__auto__ (seq
                                 (drop-while
                                   (fn fn__21482
                                     ([p1__21481#]
                                       (< (.getT ^datomic.impl.db.IDatum p1__21481#) nextT)))
                                   data))]
        (if temp__5455__auto__
          (let [newdata temp__5455__auto__]
  ...
  (defn peer-queue-exceeded
    ([]
      (let [msg__662__auto__ "Peer work queue limit exceeded, transactor may be unavailable."]
        (clojure.lang.ExceptionInfo.
          (str :db.error/peer-queue-exceeded " " msg__662__auto__)
          (error/anomalize
  ...
  (defn fail-pending-txes
    ([unsent_updates_queue pending_txes]
      ...
      (loop [seq_21489 (seq (cache/cache-keys pending_txes))
             chunk_21490 nil
             count_21491 0
             i_21492 0]
        (if (< i_21492 count_21491)
          (let [k (.nth ^clojure.lang.Indexed chunk_21490 (int i_21492))]
            (let [temp__5457__auto__ (cache/remove pending_txes k)]
              (when temp__5457__auto__
                (let [prom temp__5457__auto__] (deliver prom (transactor-unavailable)))))
            (recur seq_21489 chunk_21490 count_21491 (inc i_21492)))
          (let [temp__5457__auto__ (seq seq_21489)]
            (when temp__5457__auto__
              (let [seq_21489 temp__5457__auto__]
                (if (chunked-seq? seq_21489)
                  (let [c__5719__auto__ (chunk-first seq_21489)]
                    (recur
                      (chunk-rest seq_21489)
                      c__5719__auto__
                      (int (count c__5719__auto__))
                      (int 0)))
                  (let [k (first seq_21489)]
                    (let [temp__5457__auto__ (cache/remove pending_txes k)]
                      (when temp__5457__auto__
                        (let [prom temp__5457__auto__] (deliver prom (transactor-unavailable)))))
                    (recur (next seq_21489) nil 0 0))))))))))
  ...
  (deftype
          ...
          ([]
            (try
              (let [idxroot (index/find-index-root-id cluster)
                    ...
                    temp__5455__auto__ (adopter/adopt-index db_ref adopt_db)]
                (if temp__5455__auto__
                  (let [map__21556 temp__5455__auto__
                        map__21556 (if (seq? map__21556)
                                     (clojure.lang.PersistentHashMap/create (seq map__21556))
                                     map__21556)
                        basis_db (get map__21556 :basis-db)
                        adopted_db (get map__21556 :adopted-db)
                        msec (get map__21556 :msec)
                        iterations (get map__21556 :iterations)]
    ...
    (notify-data
      [this msg]
      (let [map__21547 msg
            map__21547 (if (seq? map__21547)
                         (clojure.lang.PersistentHashMap/create (seq map__21547))
                         map__21547)
            id (get map__21547 :id)
            data (get map__21547 :data)
            tempids (get map__21547 :tempids)
            io_stats (get map__21547 :io-stats)
            start (java.lang.System/currentTimeMillis)
            prom (cache/remove pending_txes id)
            map__21548 (meta prom)
            map__21548 (if (seq? map__21548)
                         (clojure.lang.PersistentHashMap/create (seq map__21548))
                         map__21548)
            io_context (get map__21548 :io-context)
            old_db (deref db_ref)
            new_db (let [m_21549 {:event :peer/accept-new, :id id}
                         ___8980__auto__ (let [logger (org.slf4j.LoggerFactory/getLogger
                                                        "datomic.peer")]
                                           (when (.isDebugEnabled ^org.slf4j.Logger logger)
                                             (.debug
                                               ^org.slf4j.Logger logger
                                               (logger/process (assoc m_21549 :phase :begin)))
                                             nil)
                                           nil)
                         ...
                         elapsed_21550 (- (java.lang.System/nanoTime) start__8981__auto__)
                         msec_21551 (logger/format-as-msec (long elapsed_21550))]
                     (monitor/add-stat :PeerAcceptNewMsec msec_21551)
                     (let [endmsg__8984__auto__ (merge
                                                  (assoc m_21549 :msec msec_21551 :phase :end)
    ...
    (^void gcStorage
      ...
      (do
        (let [temp__5455__auto__ (:connector (get-cstate state_ref nil))]
          (when temp__5455__auto__
            (let [connector temp__5455__auto__]
    ...
    (^java.util.concurrent.BlockingQueue txReportQueue
      ...
      (or
        ...
        (swap!
          ...
          (fn fn__21545
            ([p1__21500#]
              (if p1__21500# p1__21500# (java.util.concurrent.LinkedBlockingQueue.)))))))
    ...
    (^void release
      ...
      (do
        (locking connection-lock
         (loop [seq_21538 (seq (cache/cache-keys connection-cache))
                chunk_21539 nil
                count_21540 0
                i_21541 0]
           (if (< i_21541 count_21540)
             (let [k (.nth ^clojure.lang.Indexed chunk_21539 (int i_21541))]
               (when (= this (get connection-cache k)) (cache/remove connection-cache k))
               (recur seq_21538 chunk_21539 count_21540 (inc i_21541)))
             (let [temp__5457__auto__ (seq seq_21538)]
               (when temp__5457__auto__
                 (let [seq_21538 temp__5457__auto__]
                   (if (chunked-seq? seq_21538)
                     (let [c__5719__auto__ (chunk-first seq_21538)]
                       (recur
                         (chunk-rest seq_21538)
                         c__5719__auto__
                         (int (count c__5719__auto__))
                         (int 0)))
                     (let [k (first seq_21538)]
                       (when (= this (get connection-cache k)) (cache/remove connection-cache k))
                       (recur (next seq_21538) nil 0 0)))))))))
    ...
    (reconnect
      [this]
      (let [temp__5455__auto__ (deref state_ref 10000 nil)]
        (if temp__5455__auto__
          (let [o temp__5455__auto__] (recon/reconnect o))
      ...
      (do
        (let [logger (org.slf4j.LoggerFactory/getLogger "datomic.peer")]
          (when (.isInfoEnabled ^org.slf4j.Logger logger)
            ...
            nil)
          nil)
        (let [m_21502 {:event :peer/create-connection-impl,
                       :cluster-conf (uri/loggable-cluster-conf cluster_conf)}
              ___8980__auto__ (let [logger (org.slf4j.LoggerFactory/getLogger "datomic.peer")]
                                (when (.isDebugEnabled ^org.slf4j.Logger logger)
                                  (.debug
                                    ^org.slf4j.Logger logger
                                    (logger/process (assoc m_21502 :phase :begin)))
                                  nil)
                                nil)
              start__8981__auto__ (java.lang.System/nanoTime)
              result__8982__auto__ (try
                                            ...
                                            (try
                                              (let [updater (conn/start-updater
                                                ...
                                                (try
                                                  (do
                                                    (conn/start notifier)
                                                    (let [state (datomic.peer.ConnectionState.
                                                                  connector
                                                                  notifier
                                                                  updater
                                                                  cleanup)]
                                     ...
                                     (catch
                                       ...
                                       t__8983__auto__
                                       {:threw t__8983__auto__}))
              elapsed_21503 (- (java.lang.System/nanoTime) start__8981__auto__)
              msec_21504 (logger/format-as-msec (long elapsed_21503))]
          (let [endmsg__8984__auto__ (merge
                                       (assoc m_21502 :msec msec_21504 :phase :end)
                                       (when (:threw result__8982__auto__)
  ...
  (defn integrate-lucene
    ([db_ref q]
      (loop [tx (queue/take q)]
        (when-not (= tx :done)
          (let [vec__21578 (loop [txes [tx] ntx (queue/poll q)]
                             (if (or (= ntx :done) (nil? ntx))
                               [txes ntx]
                               (recur (conj txes ntx) (queue/poll q))))
                txes (nth vec__21578 (int 0) nil)
                ntx (nth vec__21578 (int 1) nil)
                db (deref db_ref)
                          ...
                          ([p1__21577#]
                            (db/fulltext?
                              ...
                              (java.lang.Integer/valueOf
                                (int (.getA ^datomic.impl.db.IDatum p1__21577#))))))
                        (apply concat txes)))]
            ...
            (fn fn__21590
              ([]
                (common/retry-fn
                  ...
                  :pred
                  (fn fn__21591
                    ([p1__21587#] (not (instance? datomic.peer.ConnectionState p1__21587#))))
                  :backoff
                  1000
                  :log-retry
                  common/log-retry
    ...
    (^java.util.concurrent.BlockingQueue txReportQueue
      ...
      (or
        ...
        (swap!
          ...
          (fn fn__21604
            ([p1__21598#]
              (if p1__21598# p1__21598# (java.util.concurrent.LinkedBlockingQueue.)))))))
    (^datomic.ListenableFuture transactAsync
      [this ^java.util.List txdata]
    ...
    ([dbname]
      (let [deleted (atom false)]
        (swap!
          ...
          (fn fn__21618
            ([dbs]
              (let [temp__5455__auto__ (get dbs dbname)]
                (if temp__5455__auto__
                  (let [conn temp__5455__auto__]
  ...
  (defn shutdown
    ([shutdown_clojure]
      (locking connection-lock
       (loop [seq_21627 (seq (cache/cache-keys connection-cache))
              chunk_21628 nil
              count_21629 0
              i_21630 0]
         (if (< i_21630 count_21629)
           (let [k (.nth ^clojure.lang.Indexed chunk_21628 (int i_21630))]
             (let [temp__5457__auto__ (cache/remove connection-cache k)]
               (when temp__5457__auto__
                 (let [conn temp__5457__auto__] (deref (common/async-shutdown conn)))))
             (recur seq_21627 chunk_21628 count_21629 (inc i_21630)))
           (let [temp__5457__auto__ (seq seq_21627)]
             (when temp__5457__auto__
               (let [seq_21627 temp__5457__auto__]
                 (if (chunked-seq? seq_21627)
                   (let [c__5719__auto__ (chunk-first seq_21627)]
                     (recur
                       (chunk-rest seq_21627)
                       c__5719__auto__
                       (int (count c__5719__auto__))
                       (int 0)))
                   (let [k (first seq_21627)]
                     (let [temp__5457__auto__ (cache/remove connection-cache k)]
                       (when temp__5457__auto__
                         (let [conn temp__5457__auto__] (deref (common/async-shutdown conn)))))
                     (recur (next seq_21627) nil 0 0)))))))))
      (conn/stop-all-connectors)
      (reset! cluster-stack/kv-cache-ref nil)
      (when shutdown_clojure (shutdown-agents))))
  (defn stop-connection
    ([p__21638]
      (let [map__21639 p__21638
            map__21639 (if (seq? map__21639)
                         (clojure.lang.PersistentHashMap/create (seq map__21639))
                         map__21639)
            resolved_cluster_conf map__21639
            db_id (get map__21639 :db-id)]
        (when-not :db-id
          (throw (java.lang.AssertionError. (str "Assert failed: " (pr-str :db-id)))))
        (let [temp__5457__auto__ (get connection-cache resolved_cluster_conf)]
          (when temp__5457__auto__
            (let [conn temp__5457__auto__] (.release ^datomic.Connection conn) nil)))
        (cache/clear coord/db-cache))))
  (defn get-connection
    ([cluster_conf]
      (let [m_21642 {:event :peer/get-connection,
                     :cluster-conf (uri/loggable-cluster-conf cluster_conf)}
            ___8980__auto__ (let [logger (org.slf4j.LoggerFactory/getLogger "datomic.peer")]
                              (when (.isDebugEnabled ^org.slf4j.Logger logger)
                                (.debug
                                  ^org.slf4j.Logger logger
                                  (logger/process (assoc m_21642 :phase :begin)))
                                nil)
                              nil)
            start__8981__auto__ (java.lang.System/nanoTime)
            result__8982__auto__ (try
                                   {:returned
                                    (let [temp__5455__auto__ (coord/resolve-db-name cluster_conf)]
                                      (if temp__5455__auto__
                                        (let [map__21646 temp__5455__auto__
                                              map__21646 (if (seq? map__21646)
                                                           (clojure.lang.PersistentHashMap/create
                                                             (seq map__21646))
                                                           map__21646)
                                              resolved_cluster_conf map__21646
                                              db_id (get map__21646 :db-id)]
                                          (locking connection-lock
                                           (let [temp__5455__auto__ (get
                                                                      connection-cache
                                                                      resolved_cluster_conf)]
                                             (if temp__5455__auto__
                                               (let [conn temp__5455__auto__] conn)
                                   ...
                                   (catch
                                     ...
                                     t__8983__auto__
                                     {:threw t__8983__auto__}))
            elapsed_21643 (- (java.lang.System/nanoTime) start__8981__auto__)
            msec_21644 (logger/format-as-msec (long elapsed_21643))]
        (let [endmsg__8984__auto__ (merge
                                     (assoc m_21642 :msec msec_21644 :phase :end)
                                     (when (:threw result__8982__auto__)
                                       {:threw (class (:threw result__8982__auto__))}))
              logger (org.slf4j.LoggerFactory/getLogger "datomic.peer")]
          (when (.isDebugEnabled ^org.slf4j.Logger logger)
  ...
  (defn revert-to-log-version-1
    ([p__21658]
      (let [map__21659 p__21658
            map__21659 (if (seq? map__21659)
                         (clojure.lang.PersistentHashMap/create (seq map__21659))
                         map__21659)
            db_uri (get map__21659 :db-uri)
  ...
  (defn transact-all
    ([conn txdata]
      (loop [seq_21663 (seq txdata) chunk_21664 nil count_21665 0 i_21666 0]
        (if (< i_21666 count_21665)
          (let [tx (.nth ^clojure.lang.Indexed chunk_21664 (int i_21666))]
            (deref (.transact ^datomic.Connection conn ^java.util.List tx))
            (recur seq_21663 chunk_21664 count_21665 (inc i_21666)))
          (let [temp__5457__auto__ (seq seq_21663)]
            (when temp__5457__auto__
              (let [seq_21663 temp__5457__auto__]
                (if (chunked-seq? seq_21663)
                  (let [c__5719__auto__ (chunk-first seq_21663)]
                    (recur
                      (chunk-rest seq_21663)
                      c__5719__auto__
                      (int (count c__5719__auto__))
                      (int 0)))
                  (let [tx (first seq_21663)]
                    (deref (.transact ^datomic.Connection conn ^java.util.List tx))
                    (recur (next seq_21663) nil 0 0))))))))
      :ok))
  (defn seq-iterator
    ([aseq]
      (let [a (atom aseq)]
  ...
  (defmethod pprint/simple-dispatch datomic.db.Db fn__21677 ([x] (pr x)))
  (def initialize
   (delay
     (when (and
  ...
  (defn ensure-schema-level
    ([conn]
      (let [db_before (db conn)
            ...
            temp__5457__auto__ (seq (db/tx-data-for-latest-schema-level db_before))]
        (when temp__5457__auto__
          (let [txes temp__5457__auto__]
            (let [logger (org.slf4j.LoggerFactory/getLogger "datomic.peer")]
              (when (.isInfoEnabled ^org.slf4j.Logger logger)
                (.info
                  ^org.slf4j.Logger logger
                ...
                nil)
              nil)
            (loop [seq_21682 (seq txes) chunk_21683 nil count_21684 0 i_21685 0]
              (if (< i_21685 count_21684)
                (let [tx (.nth ^clojure.lang.Indexed chunk_21683 (int i_21685))]
                  (transact conn tx)
                  (recur seq_21682 chunk_21683 count_21684 (inc i_21685)))
                (let [temp__5457__auto__ (seq seq_21682)]
                  (when temp__5457__auto__
                    (let [seq_21682 temp__5457__auto__]
                      (if (chunked-seq? seq_21682)
                        (let [c__5719__auto__ (chunk-first seq_21682)]
                          (recur
                            (chunk-rest seq_21682)
                            c__5719__auto__
                            (int (count c__5719__auto__))
                            (int 0)))
                        (let [tx (first seq_21682)]
                          (transact conn tx)
                          (recur (next seq_21682) nil 0 0)))))))))
          nil))))
  (defn connect-uri
    ([uri]
      (deref initialize)
  ...
  (defn administer-system
    ([p__21691]
      (let [map__21692 p__21691
            map__21692 (if (seq? map__21692)
                         (clojure.lang.PersistentHashMap/create (seq map__21692))
                         map__21692)
            uri (get map__21692 :uri)
            action (get map__21692 :action)]
        (cond
          (nil? action) (do
                          (throw (java.lang.IllegalArgumentException. "Invalid options map."))
                          nil)
          (and (= action :upgrade-schema) (nil? uri)) (do
                                                        (throw
                                                          (java.lang.IllegalArgumentException.
                                                            "Invalid options map."))
                                                        nil)
          (and (= action :upgrade-schema) uri) (let [conn (connect-uri uri)]
                                                 (ensure-schema-level conn)
                                                 :completed)
          (= action :release-object-cache) (let [system_cache (domain/system-cache)]
                                             (cache/clear system_cache)
                                             :completed)
          :else (do (throw (java.lang.IllegalArgumentException. "Invalid options map.")) nil)))))
  (defn send-admin-request
    ([cluster_conf request arg]
      (let [m_21696 {:event :peer/transactor-admin-request,
            ...
            ___8980__auto__ (let [logger (org.slf4j.LoggerFactory/getLogger "datomic.peer")]
                              (when (.isDebugEnabled ^org.slf4j.Logger logger)
                                (.debug
                                  ^org.slf4j.Logger logger
                                  (logger/process (assoc m_21696 :phase :begin)))
                                nil)
                              nil)
            start__8981__auto__ (java.lang.System/nanoTime)
            result__8982__auto__ (try
                                   ...
                                   (catch
                                     ...
                                     t__8983__auto__
                                     {:threw t__8983__auto__}))
            elapsed_21697 (- (java.lang.System/nanoTime) start__8981__auto__)
            msec_21698 (logger/format-as-msec (long elapsed_21697))]
        (let [endmsg__8984__auto__ (merge
                                     (assoc m_21696 :msec msec_21698 :phase :end)
                                     (when (:threw result__8982__auto__)
                                       {:threw (class (:threw result__8982__auto__))}))
              logger (org.slf4j.LoggerFactory/getLogger "datomic.peer")]
          (when (.isDebugEnabled ^org.slf4j.Logger logger)
    ...
    ([uri]
      (let [cluster_conf (uri/parse-db uri)
        ...
        (if (= protocol :mem)
          ...
          (let [temp__5455__auto__ (coord/resolve-db-name cluster_conf)]
            (if temp__5455__auto__
              (let [map__21708 temp__5455__auto__
                    map__21708 (if (seq? map__21708)
                                 (clojure.lang.PersistentHashMap/create (seq map__21708))
                                 map__21708)
                    resolved_cluster_conf map__21708
                    db_id (get map__21708 :db-id)]
                (locking connection-lock
