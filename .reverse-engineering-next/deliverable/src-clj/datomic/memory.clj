(do
    ...
    (clojure.core/with-loading-context
      (do
        ...
        (clojure.core/require
          ...
          ['datomic.common :as 'common]
          ['clojure.string :as 'str]))))
  (def MINIMUM_VM_SIZE (long (* 96 1048576)))
  (reset-meta! #'MINIMUM_VM_SIZE (assoc {:const true, :column 1} :name 'MINIMUM_VM_SIZE :ns *ns*))
  (def SEGMENT_SIZE (long (* 64 1024)))
  (reset-meta! #'SEGMENT_SIZE (assoc {:const true, :column 1} :name 'SEGMENT_SIZE :ns *ns*))
  (def DATOMS_PER_SEGMENT 8000)
  (reset-meta!
    #'DATOMS_PER_SEGMENT
    (assoc {:const true, :column 1} :name 'DATOMS_PER_SEGMENT :ns *ns*))
  (def DATOM_OBJECT_SIZE 60)
  (reset-meta!
    #'DATOM_OBJECT_SIZE
    (assoc {:const true, :column 1} :name 'DATOM_OBJECT_SIZE :ns *ns*))
  (def SEGMENT_OBJECT_SIZE (long (* 8000 60)))
  (reset-meta!
    #'SEGMENT_OBJECT_SIZE
    (assoc {:const true, :column 1} :name 'SEGMENT_OBJECT_SIZE :ns *ns*))
  (def TRANSACTOR_SPARE_MEM 0)
  (reset-meta!
    #'TRANSACTOR_SPARE_MEM
    (assoc {:const true, :column 1} :name 'TRANSACTOR_SPARE_MEM :ns *ns*))
  (defn ram->bytes
    ([ram]
  ...
  (def peer-settings transactor-settings)
  (def aws-peer-settings aws-transactor-settings)
