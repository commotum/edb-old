(do
  ...
  (deftype
    ...
    (get
      [this key consistent?]
      (let [temp__5457__auto__ (.get ^java.util.Map cache key)]
        (when temp__5457__auto__
          (let [ret temp__5457__auto__
                ret (let [temp__5455__auto__ (:v ret)]
    ...
    (put
      ...
      (when (let [put_when (fn put_when
                             ([k val emap]
                               (let [temp__5457__auto__ (.getVersioned
                                                          ^org.infinispan.client.hotrod.RemoteCache cache
                                                          k)]
                                 (when temp__5457__auto__
                                   (let [vv temp__5457__auto__
                                         ver (.getVersion
                                     ...
                                     (when (= emap (select-keys oldv (keys emap)))
                                       (or
                                         ...
                                         (recur k val emap))))))))
                  map__17802 val_map
                  map__17802 (if (seq? map__17802)
                               (clojure.lang.PersistentHashMap/create (seq map__17802))
                               map__17802)
                  id (clojure.core/get map__17802 :id)
                  ensure (clojure.core/get map__17802 :ensure)
  ...
  (defn kv-infinispan
    ([endpoint]
      (let [map__17813 endpoint
            map__17813 (if (seq? map__17813)
                         (clojure.lang.PersistentHashMap/create (seq map__17813))
                         map__17813)
            host (clojure.core/get map__17813 :host)
            port (clojure.core/get map__17813 :port)
