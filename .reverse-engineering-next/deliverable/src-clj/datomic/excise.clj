(do
  ...
  (defn component-es-set
    ([db e via_attrs]
      (let [es #{e}
            G__14764 [e]
            vec__14765 G__14764
            seq__14766 (seq vec__14765)
            first__14767 (first seq__14766)
            seq__14766 (next seq__14766)
            check first__14767
            more seq__14766
            via via_attrs]
        (loop [es es G__14764 G__14764 via via]
          (let [es es
                vec__14768 G__14764
                seq__14769 (seq vec__14768)
                first__14770 (first seq__14769)
                seq__14769 (next seq__14769)
                check first__14770
                more seq__14769
                via via]
            (if check
  ...
  (def bootids (into #{} (vals db/BOOT-IDS)))
  (reset-meta! #'bootids (assoc {:private true, :column 1} :name 'bootids :ns *ns*))
  (defn keeper?
    ([d]
      (or
  ...
  (defn pred-and-extent
    ([db spec]
      (let [id (fn id ([p1__14811#] (or (:db/id p1__14811#) (db/resolve-id db p1__14811#))))
            ...
            ref? (fn ref_QMARK_ ([p1__14812#] (= (.-vtypeid (db/attribute db p1__14812#)) 20)))
  ...
  (reset-meta!
    #'pred
    (assoc
      {:private true, :arglists (clojure.core/list ['db 'spec]), :column 1}
      :name
      'pred
      :ns
      *ns*))
          ...
          ([m spec]
            (let [e (datomic.excise/target db spec)]
              (if (datomic.excise/e-target? e)
                (let [vec__14851 (datomic.excise/pred-and-extent db spec)
                      epred (nth vec__14851 (int 0) nil)
                      es (nth vec__14851 (int 1) nil)]
                  (reduce (fn fn__14854 ([m e] (update m e conj epred))) m es))
                m))))
        {}
        specs)))
  (reset-meta!
    #'create-e->xpreds
    (assoc
      {:private true, :arglists (clojure.core/list ['db 'specs]), :column 1}
      :name
      'create-e->xpreds
      :ns
      *ns*))
          ...
          (ep-remove?
            ...
            (let [temp__5457__auto__ (concat
              ...
              (when temp__5457__auto__
                (let [xpreds temp__5457__auto__]
                  (some
                    (fn fn__14860 ([p1__14858#] (datomic.excise/ep-remove? p1__14858# d)))
                    xpreds)))))
    ...
    ([db specs]
      (let [a_>xpreds (datomic.excise/create-a->xpreds db specs)]
        (reify
          ...
          (ep-remove?
            [this d]
            (let [temp__5457__auto__ (get a_>xpreds (:a d))]
              (when temp__5457__auto__
                (let [xpreds temp__5457__auto__]
                  (some
                    (fn fn__14872 ([p1__14868#] (datomic.excise/ep-remove? p1__14868# d)))
                    xpreds)))))
          (ep-datoms
            [this]
            (mapcat
              (fn fn__14870 ([xpreds] (mapcat datomic.excise/ep-datoms xpreds)))
              (vals a_>xpreds)))))))
  (defn create-xpreds
    ([db specs]
      (when (seq specs)
