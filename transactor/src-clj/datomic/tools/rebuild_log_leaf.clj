(do
  ...
  (s/def-impl
    ...
    (s/map-spec-impl
      {:req-un nil,
       ...
       :gfn nil,
       :pred-exprs
       [(fn fn__32659 ([G__32658] (map? G__32658)))
        (fn fn__32661 ([G__32658] (contains? G__32658 :datomic.log/leaf)))
        (fn fn__32663 ([G__32658] (contains? G__32658 :datomic.log/leaf-key)))
        (fn fn__32665
          ([G__32658] (contains? G__32658 :datomic.tools.rebuild-log-leaf/bounding-segments)))],
       :keys-pred
       (fn fn__32667
         ([G__32658]
           (and
             (map? G__32658)
             (contains? G__32658 :datomic.log/leaf)
             (contains? G__32658 :datomic.log/leaf-key)
             (contains? G__32658 :datomic.tools.rebuild-log-leaf/bounding-segments)))),
       :opt-keys [],
       :req-specs
       [:datomic.log/leaf :datomic.log/leaf-key :datomic.tools.rebuild-log-leaf/bounding-segments],
       :req
      ...
      (into
        ...
        (first
          (filter
            (fn fn__32674
              ([p__32673]
                (let [vec__32675 p__32673
                      map__32678 (nth vec__32675 (int 0) nil)
                      map__32678 (if (seq? map__32678)
                                   (if (next map__32678)
                                     (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                                       (to-array map__32678))
                                     (if (seq map__32678) (first map__32678) {}))
                                   map__32678)
                      uuid (get map__32678 :uuid)]
                  (= (str uuid) k))))
            (partition
              2
              1
            ...
            (let [repair_id (str (common/rand-uuid))
              ...
              {:leaf-txes (java.lang.Integer/valueOf (int (count leaf))),
               ...
               (reduce
                 ...
                 (map
                   (fn fn__32702
                     ([p1__32699#] (java.lang.Integer/valueOf (int (count (:data p1__32699#))))))
                   leaf)),
               :leaf-size (msize/memory-size leaf),
               :leaf-id repair_id,
               :install-cmd
    ...
    ([uri rebuild]
      ...
      (let [cr (tools/connection-resources uri)
        ...
        (if (= bounds (:datomic.tools.rebuild-log-leaf/bounding-segments rebuild))
          (let [vec__32709 (repeatedly (fn fn__32715 ([] (common/rand-uuid))))
                root_id (nth vec__32709 (int 0) nil)
                dir_id (nth vec__32709 (int 1) nil)
                leaf_id (nth vec__32709 (int 2) nil)
                ...
                vec__32712 (when (= :created leaf_res dir_res root_res)
                             (race-to-adopt cr root_id 10))
                retries (nth vec__32712 (int 0) nil)
                tail_desc (nth vec__32712 (int 1) nil)]
            {:before path,
             :after
             {:root-id root_id,
              :root root,
  ...
  (defn crosscheck-log
    ([uri]
      (let [cr (tools/connection-resources uri)
            map__32718 (tools/db-resources cr)
            map__32718 (if (seq? map__32718)
                         (if (next map__32718)
                           (clojure.lang.PersistentArrayMap/createAsIfByAssoc
                             (to-array map__32718))
                           (if (seq map__32718) (first map__32718) {}))
                         map__32718)
            db (get map__32718 :db)
            log (get map__32718 :log)
            ct (atom 0)]
        (int/crosscheck-log
          log
          db
