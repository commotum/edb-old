...
fn seed_users(txr: &mut SqliteTransactor) -> (i64, i64) {
    ...
    txr.install_attribute(&a_user_age).unwrap();
    ...
    ops.push(serde_json::json!(["add", "temp:u1", ":user/age", 30]));
    ops.push(serde_json::json!(["add", "temp:u2", ":user/id", "U2"]));
    ops.push(serde_json::json!(["add", "temp:u2", ":user/name", "Bob"]));
    ops.push(serde_json::json!(["add", "temp:u2", ":user/age", 40]));
    for i in 0..1200 {
...
fn rows_as_ids(value: &serde_json::Value) -> Vec<i64> {
    value
        .as_array()
        .unwrap_or(&vec![])
        .iter()
        .filter_map(|row| row.as_array().and_then(|r| r.first()).and_then(|v| v.as_i64()))
        .collect()
}
...
fn find_value_variable() {
    ...
    assert!(rows.iter().any(|row| row.as_array().and_then(|r| r.first()).and_then(|v| v.as_str()) == Some("Bob")));
}
...
fn find_entity_with_placeholder() {
    ...
    let mut txr = SqliteTransactor::open(&path).expect("open");
    let (alice, bob) = seed_users(&mut txr);
...
fn predicate_filters_results() {
    let path = tmp_db_path("edn_predicate");
    let mut txr = SqliteTransactor::open(&path).expect("open");
    let (_, bob) = seed_users(&mut txr);

    let res = execute_edn(&path, "[:find ?e :where [?e :user/age ?age] [(> ?age 35)]]").expect("query");
    let rows = rows_as_ids(&res);
    assert_eq!(rows, vec![bob]);
}

#[test]
fn or_clause_unions_bindings() {
    let path = tmp_db_path("edn_or_clause");
    let mut txr = SqliteTransactor::open(&path).expect("open");
    let (alice, bob) = seed_users(&mut txr);

    let res = execute_edn(&path, "[:find ?e :where (or [?e :user/name \"Alice\"] [?e :user/name \"Bob\"]) ]").expect("query");
    let mut rows = rows_as_ids(&res);
    rows.sort_unstable();
    assert_eq!(rows, vec![alice, bob]);
}

#[test]
fn not_clause_filters_bindings() {
    let path = tmp_db_path("edn_not_clause");
    let mut txr = SqliteTransactor::open(&path).expect("open");
    let (alice, _) = seed_users(&mut txr);

    let res = execute_edn(
        &path,
        "[:find ?e :where [?e :user/name ?n] (not [?e :user/age 40]) ]",
    )
    .expect("query");
    let rows = rows_as_ids(&res);
    assert_eq!(rows, vec![alice]);
