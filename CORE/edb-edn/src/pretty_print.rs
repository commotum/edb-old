...
impl Value {
    ...
    where A: pretty::DocAllocator<'a>, T: Into<Cow<'a, str>>, I: IntoIterator<Item=&'a Value> {
        ...
        while let Some(v) = iter.next() {
            parts.push(v.as_doc(allocator));
            if iter.peek().is_some() {
                parts.push(allocator.line());
            }
        }
        allocator.text(open)
            .append(allocator.concat(parts).nest(n))
    ...
    fn as_doc<'a, A>(&'a self, pp: &'a A) -> pretty::DocBuilder<'a, A>
        ...
        match *self {
            ...
            Value::Map(ref vs) => {
                let mut parts = Vec::new();
                let mut iter = vs.iter().rev().peekable();
                while let Some((k, v)) = iter.next() {
                    parts.push(k.as_doc(pp).append(pp.space()).append(v.as_doc(pp)).group());
                    if iter.peek().is_some() {
                        parts.push(pp.line());
                    }
                }
                pp.text("{")
                    .append(pp.concat(parts).nest(1))
                    .append(pp.text("}"))
                    .group()
            }
