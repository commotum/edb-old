...
impl FromValue<FnArg> for FnArg {
    fn from_value(v: &ValueAndSpan) -> Option<FnArg> {
        ...
        match v.inner {
            ...
            Decimal(ref x) =>
                Some(FnArg::Constant(NonIntegerConstant::Decimal(x.clone()))),
            Text(ref x) =>
                // TODO: intern strings. #398.
...
impl FromValue<PatternValuePlace> for PatternValuePlace {
    fn from_value(v: &ValueAndSpan) -> Option<PatternValuePlace> {
        match v.inner {
            ...
            SpannedValue::Decimal(ref x) =>
                Some(PatternValuePlace::Constant(NonIntegerConstant::Decimal(x.clone()))),
...
///
/// ```rust
/// # use edb_edn::query::{Element, FindSpec, Variable};
///
/// # fn main() {
///
///   let elements = vec![
...
impl Binding {
    ...
    ///
    /// ```
    /// use edb_edn::query::{Binding,Variable,VariableOrPlaceholder};
    /// use std::rc::Rc;
    ///
