...
impl From<SpannedValue> for Value {
    fn from(src: SpannedValue) -> Value {
        match src {
            ...
            SpannedValue::Decimal(v) => Value::Decimal(v),
...
///
/// ```
/// # use edb_edn::to_symbol;
/// # use edb_edn::SpannedValue;
/// # use edb_edn::types::Value;
/// # use edb_edn::symbols;
...
/// assert_eq!(Value::from(value), to_symbol!("foo", "bar", Value));
///
/// let value = to_symbol!(None, "baz", SpannedValue);
/// assert_eq!(Value::from(value), to_symbol!(None, "baz", Value));
/// ```
#[macro_export]
macro_rules! to_symbol {
    ( $namespace:expr, $name:expr, $t:tt ) => {{
        let ns: Option<&str> = $namespace.into();
        ns.map_or_else(
...
/// assert_eq!(Value::from(value), to_keyword!("foo", "bar", Value));
///
/// let value = to_keyword!(None, "baz", SpannedValue);
/// assert_eq!(Value::from(value), to_keyword!(None, "baz", Value));
/// ```
#[macro_export]
...
macro_rules! def_common_value_methods {
    ( $t:tt<$tchild:tt> ) => {
        ...
        def_as_ref!(as_decimal, $t::Decimal, BigDecimal);
        ...
        def_into!(into_decimal, $t::Decimal, BigDecimal,);
        ...
        def_from_option!(from_decimal, $t, $t::Decimal, &str, |src: &str| src.parse::<BigDecimal>().ok());
        ...
        fn precedence(&self) -> i32 {
            match *self {
                ...
                $t::Uuid(_) => 8,
                $t::PlainSymbol(_) => 9,
                $t::NamespacedSymbol(_) => 10,
                $t::Keyword(ref k) if !k.is_namespaced() => 11,
                $t::Keyword(_) => 12,
                $t::Vector(_) => 13,
                $t::List(_) => 14,
                $t::Set(_) => 15,
...
macro_rules! def_common_value_ord {
    ( $t:tt, $value:expr, $other:expr ) => {
        match ($value, $other) {
            ...
            (&$t::Decimal(ref a), &$t::Decimal(ref b)) => b.cmp(a),
...
mod test {
    ...
    fn test_value_from() {
        ...
        assert_eq!(Value::from_decimal("42.0").unwrap().to_string(), "42.0M");
    }
    ...
    fn test_print_edn() {
        ...
        let string = "[ 1 2 ( 3.14 ) #{ 4N } 12.5M { foo/bar 42 :baz/boz 43 } [ ] :five :six/seven eight nine/ten true false nil #f NaN #f -Infinity #f +Infinity ]";
        ...
        let data = Value::Vector(vec![
            ...
            Value::from_decimal("12.5").unwrap(),
    ...
    fn test_ord() {
        ...
        assert_eq!(Value::from_decimal("1").cmp(&Value::from_decimal("2")), Ordering::Greater);
