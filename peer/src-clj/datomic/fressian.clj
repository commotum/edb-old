(do
  ...
  (defn fressian
    ([out obj & p__12173]
      (let [map__12174 p__12173
            map__12174 (if (seq? map__12174)
                         (clojure.lang.PersistentHashMap/create (seq map__12174))
                         map__12174)
            handlers (get map__12174 :handlers)
            footer (get map__12174 :footer)
            os (jio/output-stream out)]
        (try
    ...
    ([in & p__12176]
      (let [map__12177 p__12176
            map__12177 (if (seq? map__12177)
                         (clojure.lang.PersistentHashMap/create (seq map__12177))
                         map__12177)
            handlers (get map__12177 :handlers)
            footer (get map__12177 :footer)
            fin (create-reader in handlers (boolean footer))
            result (.readObject ^org.fressian.Reader fin)]
        (when footer (.validateFooter ^org.fressian.Reader fin) nil)
        result)))
  (defn byte-buf
    ([obj & options]
  ...
  (extend
    ...
    {:take (fn fn__12181 ([reader] (.readObject ^org.fressian.FressianReader reader)))})
  (defn read-batch
    ([fin]
  ...
  (defn read-seq
    ([readable handler_lookup]
      (let [map__12186 (queue/queue-seq 100)
            map__12186 (if (seq? map__12186)
                         (clojure.lang.PersistentHashMap/create (seq map__12186))
                         map__12186)
            fill (get map__12186 :fill)
            done (get map__12186 :done)
            drain (get map__12186 :drain)]
        (future-call
          (fn fn__12187
            ([]
              (try
                (let [s (jio/input-stream readable)]
                  (try
                    (do
                        ...
                        (loop []
                          (if (= (.available ^java.io.InputStream s) 0)
                            ...
                            (do
                              ...
                              (recur)))))
                      nil)
                    (finally (do (.close ^java.io.InputStream s) nil))))
        ...
        ([val]
          (let [start__9163__auto__ (java.lang.System/nanoTime)
                result__9164__auto__ (try
                                       (let [is (org.fressian.impl.ByteBufferInputStream.
                                                  ^java.nio.ByteBuffer val)]
                                         (try
                                           (let [gz (java.util.zip.GZIPInputStream.
                                               ...
                                               (let [bs (stream/buffered-input-stream gz)]
                                                 (try
                                                   (let [fr (create-reader bs read_lookup false)
                                                     ...
                                                     obj)
                                                   (finally
                                                     (do (.close ^java.io.InputStream bs) nil))))
                                               (finally
                                                 (do
                                                   (.close ^java.util.zip.GZIPInputStream gz)
                                                   nil))))
