(do
  ...
  (extend
    ...
    TreeWalker
    {:child-node-ids (fn fn__19708 ([root] (map str (vals (:attrmap root))))),
     :subtrees
     (fn fn__19710 ([root ids_>nodes] (^clojure.lang.IFn ids_>nodes (child-node-ids root))))})
  (extend
    datomic.clusterfs.ClusterFS
    TreeWalker
    {:child-node-ids (fn fn__19712 ([clusterfs] (clusterfs/all-keys clusterfs))),
     :subtrees (fn fn__19714 ([_ _] nil))})
  (extend
    datomic.index.RootNode
    TreeWalker
    {:child-node-ids (fn fn__19716 ([rn] (map str (.-dirids ^datomic.index.RootNode rn)))),
     :subtrees
     (fn fn__19718 ([rn ids_>nodes] (^clojure.lang.IFn ids_>nodes (child-node-ids rn))))})
  (extend
    datomic.index.DirNode
    TreeWalker
    {:child-node-ids (fn fn__19720 ([dn] (map str (.-segids ^datomic.index.DirNode dn)))),
     :subtrees (fn fn__19722 ([_ _] nil))})
  (deftype
    Node
    [id walker]
  ...
  (defn create-node
    ([id lookup allow_missing?]
      ...
      (let [temp__5457__auto__ (lookup-val lookup id allow_missing?)]
        (when temp__5457__auto__ (let [v temp__5457__auto__] (->Node id v))))))
        ...
        (fn fn__19733
          ([ids]
            (remove
              nil?
              (map
                (fn fn__19734 ([p1__19732#] (create-node p1__19732# lookup allow_missing?)))
                ids))))))
  ...
  (defn create-parent-node
    ([id walker lookup allow_missing?]
      (let [ids_>nodes (create-ids->nodes lookup allow_missing?)
            temp__5457__auto__ (lookup-val lookup id allow_missing?)]
        (when temp__5457__auto__
          (let [top temp__5457__auto__]
  ...
  (defn tree-node-ids
    ([node ids_>nodes]
      (concat
        (mapcat
          (fn fn__19741 ([p1__19740#] (tree-node-ids p1__19740# ids_>nodes)))
      ...
      (reify
        ...
        (subtrees
          ...
          (map
            (fn fn__19751
              ([p__19750]
                (let [map__19752 p__19750
                      map__19752 (if (seq? map__19752)
                                   (clojure.lang.PersistentHashMap/create (seq map__19752))
                                   map__19752)
                      uuid (get map__19752 :uuid)
                      temp__5457__auto__ (lookup-val lookup (str uuid) allow_missing?)]
                  (when temp__5457__auto__
                    (let [v temp__5457__auto__]
